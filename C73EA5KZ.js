...
</code></pre><p>If you are using TypeScript, you can infer the type of the value that the shape describes:</p><pre><code class="language-ts"><span class="pl-k">type</span> <span class="pl-en">User</span> <span class="pl-k">=</span> <span class="pl-en">d</span>.<span class="pl-en">InferInput</span>&#x3C;<span class="pl-k">typeof</span> <span class="pl-smi">userShape</span>>;
...
</code></pre><p>The <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parse"><code>parse</code><span class="external"></span></a> method takes an input value and returns an output value, or throws a <a href="#validation-errors">validation error</a> if parsing fails:</p><pre><code class="language-ts"><span class="pl-smi">shape</span>.<span class="pl-c1">parse</span>(<span class="pl-c1">42</span>);
...
</code></pre><p>It isn't always convenient to write a try-catch blocks to handle validation errors. Use the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#try"><code>try</code><span class="external"></span></a> method in such cases:</p><pre><code class="language-ts"><span class="pl-smi">shape</span>.<span class="pl-en">try</span>(<span class="pl-c1">42</span>);
...
</code></pre><p>Read more about issues in <a href="#validation-errors">Validation errors</a> section.</p><p>Sometimes you don't care about validation errors, and want a default value to be returned if things go south. Use the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parseordefault"><code>parseOrDefault</code><span class="external"></span></a> method for that:</p><pre><code class="language-ts"><span class="pl-smi">shape</span>.<span class="pl-en">parseOrDefault</span>(<span class="pl-c1">42</span>);
<span class="pl-c">// ⮕ 42</span>
...
</code></pre><p>If you need a fallback value for a nested shape consider using the <a href="#fallback-value"><code>catch</code></a> method.</p><p>For <a href="#async-shapes">asynchronous shapes</a> there's an alternative for each of those methods: <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parseasync"><code>parseAsync</code><span class="external"></span></a>, <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#tryasync"><code>tryAsync</code><span class="external"></span></a>, and <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parseordefaultasync"><code>parseOrDefaultAsync</code><span class="external"></span></a>.</p><p>Methods listed in this section can be safely detached from the shape instance:</p><pre><code class="language-ts"><span class="pl-k">const</span> { <span class="pl-c1">parseOrDefault</span> } <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">string</span>();
...
</code></pre><p>Infer the input and output types of <code>shape2</code>:</p><pre><code class="language-ts"><span class="pl-k">type</span> <span class="pl-en">Shape2Input</span> <span class="pl-k">=</span> <span class="pl-en">d</span>.<span class="pl-en">InferInput</span>&#x3C;<span class="pl-k">typeof</span> <span class="pl-smi">shape2</span>>;
<span class="pl-c">// ⮕ string | undefined</span>

<span class="pl-k">type</span> <span class="pl-en">Shape2Output</span> <span class="pl-k">=</span> <span class="pl-en">d</span>.<span class="pl-en">InferOutput</span>&#x3C;<span class="pl-k">typeof</span> <span class="pl-smi">shape2</span>>;
...
</code></pre><h1 id="validation-errors"><a class="markdown-permalink" href="#validation-errors"><span class="icon icon-link"></span></a>Validation errors</h1><p>Validation errors which are thrown by <a href="#parsing-and-trying">parsing methods</a>, and <a href="https://smikhalevski.github.io/doubter/interfaces/core.Err.html"><code>Err</code><span class="external"></span></a> objects returned by <code>try</code> and <code>tryAsync</code> methods have the <code>issues</code> property which holds an array of validation issues:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">object</span>({ age: <span class="pl-smi">d</span>.<span class="pl-en">number</span>() });
...
</code></pre><p>The <code>result</code> contains the <a href="https://smikhalevski.github.io/doubter/interfaces/core.Err.html"><code>Err</code><span class="external"></span></a> object with the array of issues:</p><pre><code class="language-json5">{
...
</code></pre><p>Operations added via <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#addoperation"><code>addOperation</code><span class="external"></span></a> must return a <a href="https://smikhalevski.github.io/doubter/types/core.Result.html"><code>Result</code><span class="external"></span></a>:</p><ul><li><code>null</code> if the value is valid and unchanged;</li><li>an <a href="https://smikhalevski.github.io/doubter/interfaces/core.Ok.html"><code>Ok</code><span class="external"></span></a> object (as in example above) if the value was transformed;</li><li>an array of <a href="https://smikhalevski.github.io/doubter/interfaces/core.Issue.html"><code>Issue</code><span class="external"></span></a> objects if the operation has failed.</li></ul><p>Multiple operations can be added to shape, and they are executed in the same order they were added. To access all operations that were added use the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#operations"><code>operations</code><span class="external"></span></a> property.</p><p>In contrast to <a href="#conversions">conversions</a> and <a href="#shape-piping">pipes</a>, operations don't change the base shape. So you can mix them with other operations that belong to the prototype of the base shape:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape2</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>
...
</code></pre><p>Adding an async operation to the shape, makes shape itself async, so use <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parseasync"><code>parseAsync</code><span class="external"></span></a>, <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#tryasync"><code>tryAsync</code><span class="external"></span></a>, or <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parseordefaultasync"><code>parseOrDefaultAsync</code><span class="external"></span></a>.</p><h2 id="checks"><a class="markdown-permalink" href="#checks"><span class="icon icon-link"></span></a>Checks</h2><p>Checks are the most common <a href="#operations">operations</a> that allow constraining the input value beyond type assertions. For example, if you want to constrain a numeric input to be greater than or equal to 5:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">check</span>(<span class="pl-v">value</span> <span class="pl-k">=></span> {
...
</code></pre><p>A check callback receives the shape output value and must return an issue or an array of issues if the value is invalid. If the value is valid, a check callback must return <code>null</code>, <code>undefined</code>, or an empty array.</p><p>Add asynchronous checks using <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#checkasync"><code>checkAsync</code><span class="external"></span></a>. This method has the same semantics as <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#check"><code>check</code><span class="external"></span></a> but returns a promise and <a href="#async-shapes">makes the shape asynchronous</a>.</p><div class="markdown-alert markdown-alert-note"><p class="markdown-alert-title"><svg class="octicon octicon-info mr-2" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8Zm8-6.5a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13ZM6.5 7.75A.75.75 0 0 1 7.25 7h1a.75.75 0 0 1 .75.75v2.75h.25a.75.75 0 0 1 0 1.5h-2a.75.75 0 0 1 0-1.5h.25v-2h-.25a.75.75 0 0 1-.75-.75ZM8 6a1 1 0 1 1 0-2 1 1 0 0 1 0 2Z"></path></svg>Note</p><p>You can <a href="#operations">parameterize</a> checks and <a href="#tolerance-for-issues">set tolerance for issues</a> the same way as any other operation.</p></div><p>Most shapes have <a href="#built-in-plugins">a set of built-in checks</a>. The check we've just implemented above is called <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#gte"><code>gte</code><span class="external"></span></a> (greater than equals):</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">gte</span>(<span class="pl-c1">5</span>);
...
</code></pre><p>In the example above, an <a href="https://smikhalevski.github.io/doubter/interfaces/core.Err.html"><code>Err</code><span class="external"></span></a> object is returned:</p><pre><code class="language-json5">{
  ok<span class="pl-k">:</span> <span class="pl-c1">false</span>,
  issues<span class="pl-k">:</span> [
    {
...
</code></pre><p>Add asynchronous refinements using <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#refineasync"><code>refineAsync</code><span class="external"></span></a>. This method has the same semantics as <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#refine"><code>refine</code><span class="external"></span></a> but returns a promise and <a href="#async-shapes">makes the shape asynchronous</a>.</p><p>Use refinements to <a href="https://www.typescriptlang.org/docs/handbook/2/narrowing.html">narrow<span class="external"></span></a> the output type of the shape:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">isMarsOrPluto</span>(<span class="pl-v">value</span><span class="pl-k">:</span> <span class="pl-c1">string</span>)<span class="pl-k">:</span> <span class="pl-v">value</span> <span class="pl-k">is</span> <span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span> <span class="pl-k">|</span> <span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span> {
...
<span class="pl-c">// ⮕ StringShape</span>
</code></pre><p>Add asynchronous alterations using <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#alterasync"><code>alterAsync</code><span class="external"></span></a>. This method has the same semantics as <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#alter"><code>alter</code><span class="external"></span></a> but returns a promise and <a href="#async-shapes">makes the shape asynchronous</a>.</p><p>Use any transformation library in conjunction with alternations:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">alter</span>(<span class="pl-c1">Math</span>.<span class="pl-c1">abs</span>).<span class="pl-en">alter</span>(<span class="pl-c1">Math</span>.<span class="pl-c1">pow</span>, { param: <span class="pl-c1">3</span> });
...
</code></pre><p>Throw a <a href="https://smikhalevski.github.io/doubter/classes/core.ValidationError.html"><code>ValidationError</code><span class="external"></span></a> inside the callback to notify parser that the conversion cannot be successfully completed:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">toNumber</span>(<span class="pl-v">input</span><span class="pl-k">:</span> <span class="pl-c1">string</span>)<span class="pl-k">:</span> <span class="pl-c1">number</span> {
...
</code></pre><p>Refer to <a href="#async-shapes">Async shapes</a> section for more details on when shapes can become asynchronous.</p><h1 id="early-return"><a class="markdown-permalink" href="#early-return"><span class="icon icon-link"></span></a>Early return</h1><p>By default, Doubter collects all issues during parsing. In some cases, you may want to halt parsing and raise a validation error as soon as the first issue was encountered. To do this, pass the <a href="https://smikhalevski.github.io/doubter/interfaces/core.ParseOptions.html#isearlyreturn"><code>isEarlyReturn</code><span class="external"></span></a> option to the <a href="#parsing-and-trying">parsing methods</a>.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">string</span>()
...
</code></pre><p>This would return the <a href="https://smikhalevski.github.io/doubter/interfaces/core.Err.html"><code>Err</code><span class="external"></span></a> object with only one issue:</p><pre><code class="language-json5">{
  ok<span class="pl-k">:</span> <span class="pl-c1">false</span>,
  issues<span class="pl-k">:</span> [
    {
      code<span class="pl-k">:</span> <span class="pl-s"><span class="pl-pds">'</span>string.max<span class="pl-pds">'</span></span>,
...
</code></pre><p><a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#annotate"><code>annotate</code><span class="external"></span></a> returns the clone of the shape with updated annotations. Annotations are merged when you add them:</p><pre><code class="language-ts"><span class="pl-smi">shape</span>.<span class="pl-en">annotate</span>({ foo: <span class="pl-s"><span class="pl-pds">'</span>bar<span class="pl-pds">'</span></span> }).<span class="pl-smi">annotations</span>;
<span class="pl-c">// ⮕ { description: 'Username', foo: 'bar' }</span>
</code></pre><p><a href="#validation-errors">Validation issues</a> have a <a href="https://smikhalevski.github.io/doubter/interfaces/core.Issue.html#meta"><code>meta</code><span class="external"></span></a> property that you can use to store an arbitrary data.</p><p>You can pass the <a href="https://smikhalevski.github.io/doubter/interfaces/core.IssueOptions.html#meta"><code>meta</code><span class="external"></span></a> option to any <a href="#checks">built-in check</a> and its value is assigned to the <code>meta</code> property of the raised validation issue.</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">gt</span>(<span class="pl-c1">5</span>, { meta: <span class="pl-s"><span class="pl-pds">'</span>Useful data<span class="pl-pds">'</span></span> });
<span class="pl-c">// ⮕ Shape&#x3C;number></span>
...
</code></pre><p>This comes handy if you want to enhance an issue with an additional data that can be used later during issues processing. For example, during <a href="#localization">localization</a>.</p><h1 id="parsing-context"><a class="markdown-permalink" href="#parsing-context"><span class="icon icon-link"></span></a>Parsing context</h1><p>Inside <a href="#operations">operation</a> callbacks, <a href="#checks">check</a> callbacks, <a href="#refinements">refinement predicates</a>, <a href="#alterations">alteration</a> callbacks, <a href="#conversions">converters</a>, <a href="#fallback-value">fallback</a> functions, and <a href="#localization">message</a> callbacks you can access options passed to the parser. The <a href="https://smikhalevski.github.io/doubter/interfaces/core.ParseOptions.html#context"><code>context</code><span class="external"></span></a> option may store an arbitrary data, which is <code>undefined</code> by default.</p><p>For example, here's how you can use context to convert numbers to formatted strings:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">convert</span>((<span class="pl-v">value</span>, <span class="pl-v">options</span>) <span class="pl-k">=></span> <span class="pl-k">new</span> <span class="pl-c1">Intl</span>.<span class="pl-en">NumberFormat</span>(<span class="pl-smi">options</span>.<span class="pl-smi">context</span>.<span class="pl-smi">locale</span>).<span class="pl-en">format</span>(<span class="pl-smi">value</span>));
...
</code></pre><h1 id="shape-piping"><a class="markdown-permalink" href="#shape-piping"><span class="icon icon-link"></span></a>Shape piping</h1><p>With shape piping you can pass the shape output to another shape.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">string</span>()
...
</code></pre><h1 id="replace-allow-and-deny-a-value"><a class="markdown-permalink" href="#replace-allow-and-deny-a-value"><span class="icon icon-link"></span></a>Replace, allow, and deny a value</h1><p>All shapes support <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#replace"><code>replace</code><span class="external"></span></a>, <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#allow"><code>allow</code><span class="external"></span></a>, and <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#deny"><code>deny</code><span class="external"></span></a> methods that change how separate literal values are processed.</p><h2 id="replace-a-value"><a class="markdown-permalink" href="#replace-a-value"><span class="icon icon-link"></span></a>Replace a value</h2><p>You can replace an input value with an output value:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape1</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">enum</span>([<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span>]).<span class="pl-c1">replace</span>(<span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Jupiter<span class="pl-pds">'</span></span>);
...
</code></pre><p>To remove a value from this enum you can use the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#deny"><code>deny</code><span class="external"></span></a> method:</p><pre><code class="language-ts"><span class="pl-smi">shape1</span>.<span class="pl-en">deny</span>(<span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span>);
...
</code></pre><p>Fallback functions receive an input value, an array of issues and <a href="https://smikhalevski.github.io/doubter/interfaces/core.ParseOptions.html">parsing options<span class="external"></span></a> (so you can access your <a href="#parsing-context">custom context</a> if needed).</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">string</span>().<span class="pl-c1">catch</span>((<span class="pl-v">input</span>, <span class="pl-v">issues</span>, <span class="pl-v">options</span>) <span class="pl-k">=></span> {
...
</code></pre><p>In some cases, it can be desirable to simulate nominal typing inside TypeScript. For instance, you may wish to write a function that only accepts an input that has been validated by Doubter. This can be achieved with unchecked refinements:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">flightCodeShape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">string</span>().<span class="pl-en">brand</span>&#x3C;{ <span class="pl-v">__brand__</span><span class="pl-k">:</span> <span class="pl-s"><span class="pl-pds">'</span>FlightCode<span class="pl-pds">'</span></span> }>();
<span class="pl-c">// ⮕ Shape&#x3C;string, FlightCode></span>

<span class="pl-k">type</span> <span class="pl-en">FlightCode</span> <span class="pl-k">=</span> <span class="pl-en">d</span>.<span class="pl-en">InferOutput</span>&#x3C;<span class="pl-k">typeof</span> <span class="pl-smi">flightCodeShape</span>>;
<span class="pl-c">// ⮕ string &#x26; { __brand__: 'FlightCode' }</span>
...
<span class="pl-en">bookTicket</span>(<span class="pl-s"><span class="pl-pds">'</span>Bill<span class="pl-pds">'</span></span>);
<span class="pl-c">// ❌ Error: Missing property __brand__</span>
...
<span class="pl-smi">yesNoShape</span>.<span class="pl-c1">parse</span>(<span class="pl-s"><span class="pl-pds">'</span>true<span class="pl-pds">'</span></span>);
<span class="pl-c">// ❌ ValidationError: type.boolean at /: Must be a boolean</span>
</code></pre><h1 id="introspection"><a class="markdown-permalink" href="#introspection"><span class="icon icon-link"></span></a>Introspection</h1><p>Doubter provides various ways to introspect your shapes at runtime. Let's start by accessing shape input types using the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#inputs"><code>inputs</code><span class="external"></span></a> property:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape1</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">or</span>([<span class="pl-smi">d</span>.<span class="pl-en">string</span>(), <span class="pl-smi">d</span>.<span class="pl-en">boolean</span>()]);
...
</code></pre><p>To detect the type of the value use <a href="https://smikhalevski.github.io/doubter/classes/core.Type.html#of"><code>Type.of</code><span class="external"></span></a>:</p><pre><code class="language-ts"><span class="pl-smi">Type</span>.<span class="pl-en">of</span>(<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>);
...
</code></pre><p>Semantics described above are applied to the <a href="https://smikhalevski.github.io/doubter/interfaces/core.IssueOptions.html#message"><code>message</code><span class="external"></span></a> option as well:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">string</span>().<span class="pl-en">length</span>(<span class="pl-c1">3</span>, { message: <span class="pl-s"><span class="pl-pds">'</span>Invalid length<span class="pl-pds">'</span></span> });
</code></pre><h2 id="override-default-messages"><a class="markdown-permalink" href="#override-default-messages"><span class="icon icon-link"></span></a>Override default messages</h2><p>Default issue messages can be overridden by <a href="https://smikhalevski.github.io/doubter/interfaces/core.ParseOptions.html#messages"><code>messages</code><span class="external"></span></a> option:</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-c1">*</span> <span class="pl-k">as</span> <span class="pl-smi">d</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter<span class="pl-pds">'</span></span>;
...
<span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">gte</span>(<span class="pl-c1">3</span>); <span class="pl-c">// ❌ gte is undefined</span>
</code></pre><h2 id="built-in-plugins"><a class="markdown-permalink" href="#built-in-plugins"><span class="icon icon-link"></span></a>Built-in plugins</h2><ul><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_array-essentials.html"><strong>plugin/array-essentials</strong><span class="external"></span></a></p><p><a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#length"><code>length</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#min"><code>min</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#max"><code>max</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#nonempty"><code>nonEmpty</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#includes"><code>includes</code><span class="external"></span></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_bigint-essentials.html"><strong>plugin/bigint-essentials</strong><span class="external"></span></a></p><p><a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#positive"><code>positive</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#negative"><code>negative</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#nonpositive"><code>nonPositive</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#nonnegative"><code>nonNegative</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#min"><code>min</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#max"><code>max</code><span class="external"></span></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_date-essentials.html"><strong>plugin/date-essentials</strong><span class="external"></span></a></p><p><a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#min"><code>min</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#max"><code>max</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#after"><code>after</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#before"><code>before</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#toisostring"><code>toISOString</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#totimestamp"><code>toTimestamp</code><span class="external"></span></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_number-essentials.html"><strong>plugin/number-essentials</strong><span class="external"></span></a></p><p><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#finite"><code>finite</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#int"><code>int</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#positive"><code>positive</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#negative"><code>negative</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#nonpositive"><code>nonPositive</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#nonnegative"><code>nonNegative</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#between"><code>between</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#gt"><code>gt</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#lt"><code>lt</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#gte"><code>gte</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#lte"><code>lte</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#min"><code>min</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#max"><code>max</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#multipleof"><code>multipleOf</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#safe"><code>safe</code><span class="external"></span></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_object-essentials.html"><strong>plugin/object-essentials</strong><span class="external"></span></a></p><p><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#plain"><code>plain</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#minkeycount"><code>minKeyCount</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#maxkeycount"><code>maxKeyCount</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#nonempty"><code>nonEmpty</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#allkeys"><code>allKeys</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#notallkeys"><code>notAllKeys</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#orkeys"><code>orKeys</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#xorkeys"><code>xorKeys</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#oxorkeys"><code>oxorKeys</code><span class="external"></span></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_set-essentials.html"><strong>plugin/set-essentials</strong><span class="external"></span></a></p><p><a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html#size"><code>size</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html#min"><code>min</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html#max"><code>max</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html#nonempty"><code>nonEmpty</code><span class="external"></span></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_string-essentials.html"><strong>plugin/string-essentials</strong><span class="external"></span></a></p><p><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#length"><code>length</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#min"><code>min</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#max"><code>max</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#regex"><code>regex</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#includes"><code>includes</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#startswith"><code>startsWith</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#endswith"><code>endsWith</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#nonblank"><code>nonBlank</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#nonempty"><code>nonEmpty</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#trim"><code>trim</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#tolowercase"><code>toLowerCase</code><span class="external"></span></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#touppercase"><code>toUpperCase</code><span class="external"></span></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_standard-schema.html"><strong>plugin/standard-schema</strong><span class="external"></span></a></p><p>Enables <a href="https://github.com/standard-schema/standard-schema#readme">Standard Schema<span class="external"></span></a> API for all shapes.</p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_object-eval.html"><strong>plugin/object-eval</strong><span class="external"></span></a></p><p>If <code>new Function</code> calls are allowed by the environment, this plugin compiles internal methods of the <code>ObjectShape</code> to boost performance.</p></li></ul><h2 id="recommended-plugins"><a class="markdown-permalink" href="#recommended-plugins"><span class="icon icon-link"></span></a>Recommended plugins</h2><ul><li><p><a href="https://github.com/smikhalevski/doubter-plugin-string-format#readme"><strong>@doubter/plugin-string-format</strong><span class="external"></span></a><br></p><p>Extends <code>StringShape</code> with email, FQDN, MIME, BIC, ISIN, Luhn, and many other format checks.</p></li></ul><h2 id="integrations"><a class="markdown-permalink" href="#integrations"><span class="icon icon-link"></span></a>Integrations</h2><p>You can combine Doubter with your favourite predicate library using <a href="#refinements">refinements</a>.</p><p>For example, create a shape that validates that input is an email using <a href="https://github.com/validatorjs/validator.js">Validator.js<span class="external"></span></a>:</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-c1">*</span> <span class="pl-k">as</span> <span class="pl-smi">d</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter<span class="pl-pds">'</span></span>;
...
</code></pre><p>You can use Doubter <a href="#alterations">alterations</a> with various utility libraries, such as <a href="https://lodash.com/">Lodash<span class="external"></span></a>:</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-c1">*</span> <span class="pl-k">as</span> <span class="pl-smi">d</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter<span class="pl-pds">'</span></span>;
...
</code></pre><h2 id="authoring-a-plugin"><a class="markdown-permalink" href="#authoring-a-plugin"><span class="icon icon-link"></span></a>Authoring a plugin</h2><p>Plugins use <a href="https://www.typescriptlang.org/docs/handbook/declaration-merging.html#module-augmentation">TypeScript's module augmentation<span class="external"></span></a> to extend functionality of shapes exported from the <a href="https://smikhalevski.github.io/doubter/classes/core.html">doubter/core<span class="external"></span></a> module.</p><p>Below is an example of how you can implement a naive email check and extend the <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html"><code>StringShape</code><span class="external"></span></a>.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">StringShape</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter/core<span class="pl-pds">'</span></span>;
...
</code></pre><p>You can use generic <a href="#operations">operations</a>, <a href="#checks">checks</a>, <a href="#refinements">refinements</a>, <a href="#alterations">alterations</a>, <a href="#conversions">conversions</a>, and any other functionality of the shape that is being extended.</p><h1 id="advanced-shapes"><a class="markdown-permalink" href="#advanced-shapes"><span class="icon icon-link"></span></a>Advanced shapes</h1><p>You can create custom shapes by extending the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html"><code>Shape</code><span class="external"></span></a> class.</p><p><code>Shape</code> has several protected methods that you can override to change different aspects of the shape logic.</p><ol><li><p><a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#_apply"><code>_apply(input, options, nonce)</code><span class="external"></span></a></p><p>Synchronous input parsing is delegated to this method. It receives an <code>input</code> that must be parsed and should return the <a href="https://smikhalevski.github.io/doubter/types/core.Result.html"><code>Result</code><span class="external"></span></a>:</p><ul><li><code>null</code> if the output value is the same as the input value;</li><li>an <a href="https://smikhalevski.github.io/doubter/interfaces/core.Ok.html"><code>Ok</code><span class="external"></span></a> object (as in example above) if the output contains a new value;</li><li>an array of <a href="https://smikhalevski.github.io/doubter/interfaces/core.Issue.html"><code>Issue</code><span class="external"></span></a> objects if parsing has failed.</li></ul></li><li><p><a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#_applyasync"><code>_applyAsync(input, options, nonce)</code><span class="external"></span></a></p><p>Asynchronous input parsing is delegated to this method. It has the same semantics as <code>_apply</code> but returns a <code>Promise</code>. You need to override this method only if you have a separate logic for async parsing.</p></li><li><p><a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#_isasync"><code>_isAsync()</code><span class="external"></span></a></p><p>The value returned from this method toggles which method is used for parsing:</p><ul><li>if <code>true</code> then <code>_applyAsync</code> would be used for parsing, and <code>_apply</code> would always throw an error;</li><li>if <code>false</code> then <code>_apply</code> can be used for parsing along with <code>_applyAsync</code>.</li></ul></li><li><p><a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#_getinputs"><code>_getInputs()</code><span class="external"></span></a></p><p>Must return an array of types and values that can be processed by the shape. Elements of the returned array don't have to be unique. Refer to <a href="#introspection">Introspection</a> section for more details about types.</p></li></ol><p>Let's create a custom shape that parses an input string as a number:</p><pre><code class="language-ts"><span class="pl-k">class</span> <span class="pl-en">NumberLikeShape</span> <span class="pl-k">extends</span> <span class="pl-en">d</span>.<span class="pl-e">Shape</span>&#x3C;<span class="pl-c1">string</span>, <span class="pl-c1">number</span>> {
...
</code></pre><h2 id="implementing-deep-partial-support"><a class="markdown-permalink" href="#implementing-deep-partial-support"><span class="icon icon-link"></span></a>Implementing deep partial support</h2><p>To enable <code>deepPartial</code> support, your shape must implement <a href="https://smikhalevski.github.io/doubter/interfaces/core.DeepPartialProtocol.html"><code>DeepPartialProtocol</code><span class="external"></span></a>.</p><pre><code class="language-ts"><span class="pl-k">class</span> <span class="pl-en">MyShape</span> <span class="pl-k">extends</span> <span class="pl-e">Shape</span> <span class="pl-k">implements</span> <span class="pl-e">DeepPartialProtocol</span>&#x3C;<span class="pl-en">MyDeepPartialShape</span>> {
...
<span class="pl-c">// ⮕ Shape&#x3C;{ foo: string }></span>
</code></pre><p>Create a shape that is constrained by a <a href="https://www.typescriptlang.org/docs/handbook/2/narrowing.html">narrowing predicate<span class="external"></span></a>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">any</span>((<span class="pl-v">value</span>)<span class="pl-k">:</span> <span class="pl-v">value</span> <span class="pl-k">is</span> <span class="pl-c1">string</span> <span class="pl-k">=></span> <span class="pl-k">typeof</span> <span class="pl-smi">value</span> <span class="pl-k">===</span> <span class="pl-s"><span class="pl-pds">'</span>string<span class="pl-pds">'</span></span>);
<span class="pl-c">// ⮕ Shape&#x3C;any, string></span>
</code></pre><h1 id="array"><a class="markdown-permalink" href="#array"><span class="icon icon-link"></span></a><code>array</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.array.html"><code>d.array</code><span class="external"></span></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html"><code>ArrayShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be an array:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">array</span>();
<span class="pl-c">// ⮕ Shape&#x3C;any[]></span>
...
</code></pre><h1 id="bigint"><a class="markdown-permalink" href="#bigint"><span class="icon icon-link"></span></a><code>bigint</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.bigint.html"><code>d.bigint</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html"><code>BigIntShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be a bigint.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">bigint</span>();
...
<span class="pl-c">// ❌ ValidationError: type.bigint at /: Must be a bigint</span>
</code></pre><h1 id="boolean-bool"><a class="markdown-permalink" href="#boolean-bool"><span class="icon icon-link"></span></a><code>boolean</code>, <code>bool</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.boolean.html"><code>d.boolean</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.BooleanShape.html"><code>BooleanShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be boolean.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">boolean</span>();
<span class="pl-c">// or</span>
<span class="pl-smi">d</span>.<span class="pl-en">bool</span>();
<span class="pl-c">// ⮕ Shape&#x3C;boolean></span>
</code></pre><h2 id="coerce-to-a-boolean"><a class="markdown-permalink" href="#coerce-to-a-boolean"><span class="icon icon-link"></span></a>Coerce to a boolean</h2><p><code>null</code>, <code>undefined</code>, <code>'0'</code>, 0, and case-insensitive string <code>'false'</code> are converted to <code>false</code>:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">boolean</span>().<span class="pl-en">coerce</span>();

<span class="pl-smi">shape</span>.<span class="pl-c1">parse</span>(<span class="pl-c1">null</span>);
<span class="pl-c">// ⮕ false</span>
</code></pre><p><code>'1'</code>, 1, and case-insensitive string <code>'true'</code> are converted to <code>true</code>:</p><pre><code class="language-ts"><span class="pl-smi">shape</span>.<span class="pl-c1">parse</span>(<span class="pl-s"><span class="pl-pds">'</span>true<span class="pl-pds">'</span></span>);
<span class="pl-c">// ⮕ true</span>
...
<span class="pl-c">// ❌ ValidationError: type.boolean at /: Must be a boolean</span>
</code></pre><h1 id="const"><a class="markdown-permalink" href="#const"><span class="icon icon-link"></span></a><code>const</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.const.html"><code>d.const</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.ConstShape.html"><code>ConstShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be an exact value:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">const</span>(<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>);
...
<span class="pl-smi">shape2</span>.<span class="pl-c1">parse</span>([<span class="pl-smi">users</span>]);
<span class="pl-c">// ⮕ users</span>

<span class="pl-smi">shape2</span>.<span class="pl-c1">parse</span>(<span class="pl-k">new</span> <span class="pl-c1">Set</span>([<span class="pl-s"><span class="pl-pds">'</span>Bill<span class="pl-pds">'</span></span>]));
<span class="pl-c">// ❌ ValidationError: type.set at /: Must be equal to [object Set]</span>
</code></pre><h1 id="convert-convertasync"><a class="markdown-permalink" href="#convert-convertasync"><span class="icon icon-link"></span></a><code>convert</code>, <code>convertAsync</code></h1><p>Both <a href="https://smikhalevski.github.io/doubter/functions/core.convert.html"><code>d.convert</code><span class="external"></span></a> and <a href="https://smikhalevski.github.io/doubter/functions/core.convertAsync.html"><code>d.convertAsync</code><span class="external"></span></a> return a <a href="https://smikhalevski.github.io/doubter/classes/core.ConvertShape.html"><code>ConvertShape</code><span class="external"></span></a> instance.</p><p>Converts the input value:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">convert</span>(<span class="pl-smi">parseFloat</span>);
<span class="pl-c">// ⮕ Shape&#x3C;any, number></span>
...
<span class="pl-c">// ⮕ Shape&#x3C;any, string></span>
</code></pre><p>For more information, see <a href="#conversions">Conversions</a> section.</p><h1 id="date"><a class="markdown-permalink" href="#date"><span class="icon icon-link"></span></a><code>date</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.date.html"><code>d.date</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html"><code>DateShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be a valid date.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">date</span>();
...
<span class="pl-c">// ❌ ValidationError: type.date at /: Must be a Date</span>
</code></pre><h1 id="enum"><a class="markdown-permalink" href="#enum"><span class="icon icon-link"></span></a><code>enum</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.enum.html"><code>d.enum</code><span class="external"></span></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.EnumShape.html"><code>EnumShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be equal to one of predefined values:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">enum</span>([<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Jupiter<span class="pl-pds">'</span></span>]);
<span class="pl-c">// ⮕ Shape&#x3C;'Mars', 'Pluto', 'Jupiter'></span>
...
</code></pre><p>Or use <a href="https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-4.html#const-assertions">an object with a <code>const</code> assertion<span class="external"></span></a>:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">planets</span> <span class="pl-k">=</span> {
...
<span class="pl-smi">d</span>.<span class="pl-en">enum</span>(<span class="pl-smi">planets</span>);
<span class="pl-c">// ⮕ Shape&#x3C;'Mars', 'Pluto', 'Jupiter'></span>
...
</code></pre><p>Note that the input and output functions described by this shape have different signatures. Let's implement this function:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">inputFunction</span>(<span class="pl-v">arg</span><span class="pl-k">:</span> <span class="pl-c1">number</span>)<span class="pl-k">:</span> <span class="pl-c1">any</span> {
...
</code></pre><h1 id="instanceof"><a class="markdown-permalink" href="#instanceof"><span class="icon icon-link"></span></a><code>instanceOf</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.instanceOf.html"><code>d.instanceOf</code><span class="external"></span></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.InstanceShape.html"><code>InstanceShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be an object that is an instance of a class:</p><pre><code class="language-ts"><span class="pl-k">class</span> <span class="pl-en">User</span> {
...
</code></pre><h1 id="intersection-and"><a class="markdown-permalink" href="#intersection-and"><span class="icon icon-link"></span></a><code>intersection</code>, <code>and</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.intersection.html"><code>d.intersection</code><span class="external"></span></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.IntersectionShape.html"><code>IntersectionShape</code><span class="external"></span></a> instance.</p><p>Creates a shape that checks that the input value conforms to all shapes.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">intersection</span>([
  <span class="pl-smi">d</span>.<span class="pl-en">object</span>({
    name: <span class="pl-smi">d</span>.<span class="pl-en">string</span>(),
  }),
  <span class="pl-smi">d</span>.<span class="pl-en">object</span>({
...
</code></pre><h1 id="lazy"><a class="markdown-permalink" href="#lazy"><span class="icon icon-link"></span></a><code>lazy</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.lazy.html"><code>d.lazy</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.LazyShape.html"><code>LazyShape</code><span class="external"></span></a> instance.</p><p>With <code>lazy</code> you can declare recursive shapes. To showcase how to use it, let's create a shape that validates JSON data:</p><pre><code class="language-ts"><span class="pl-k">type</span> <span class="pl-en">JSON</span> <span class="pl-k">=</span>
...
</code></pre><p>You can <a href="https://smikhalevski.github.io/doubter/classes/core.LazyShape.html#circular">provide a callback<span class="external"></span></a> that returns a value that is used as a replacement value for circular references. Or it can throw a <a href="#validation-errors"><code>ValidationError</code></a> from the callback to indicate that circular references aren't allowed:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">userShape3</span><span class="pl-k">:</span> <span class="pl-en">d</span>.<span class="pl-en">Shape</span>&#x3C;<span class="pl-en">User</span>> <span class="pl-k">=</span> <span class="pl-smi">d</span>
  .<span class="pl-en">lazy</span>(() <span class="pl-k">=></span>
    <span class="pl-smi">d</span>.<span class="pl-en">object</span>({
...
</code></pre><h1 id="map"><a class="markdown-permalink" href="#map"><span class="icon icon-link"></span></a><code>map</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.map.html"><code>d.map</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.MapShape.html"><code>MapShape</code><span class="external"></span></a> instance.</p><p>Constrains an input to be a <code>Map</code> instance:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">map</span>(<span class="pl-smi">d</span>.<span class="pl-en">string</span>(), <span class="pl-smi">d</span>.<span class="pl-en">number</span>());
...
</code></pre><h1 id="nan"><a class="markdown-permalink" href="#nan"><span class="icon icon-link"></span></a><code>nan</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.nan.html"><code>d.nan</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.ConstShape.html"><code>ConstShape</code><span class="external"></span></a> instance.</p><p>The shape that requires an input to be <code>NaN</code>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">nan</span>();
<span class="pl-c">// ⮕ Shape&#x3C;number></span>
...
<span class="pl-c">// ⮕ Shape&#x3C;number></span>
</code></pre><h1 id="never"><a class="markdown-permalink" href="#never"><span class="icon icon-link"></span></a><code>never</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.never.html"><code>d.never</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.NeverShape.html"><code>NeverShape</code><span class="external"></span></a> instance.</p><p>The shape that always raises a validation issue regardless of an input value:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">never</span>();
<span class="pl-c">// ⮕ Shape&#x3C;never></span>
</code></pre><h1 id="not"><a class="markdown-permalink" href="#not"><span class="icon icon-link"></span></a><code>not</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.not.html"><code>d.not</code><span class="external"></span></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.ExcludeShape.html"><code>ExcludeShape</code><span class="external"></span></a> instance.</p><p>The shape that allows any value that doesn't conform the negated shape:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">not</span>(<span class="pl-smi">d</span>.<span class="pl-en">string</span>());
<span class="pl-c">// ⮕ Shape&#x3C;any></span>
...
<span class="pl-c">// ❌ ValidationError: any.exclude at /: Must not conform the excluded shape</span>
</code></pre><p>More about exclusions in the <a href="#exclude-a-shape">Exclude a shape</a> section.</p><h1 id="null"><a class="markdown-permalink" href="#null"><span class="icon icon-link"></span></a><code>null</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.null.html"><code>d.null</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.ConstShape.html"><code>ConstShape</code><span class="external"></span></a> instance.</p><p>The shape that requires an input to be <code>null</code>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">null</span>();
<span class="pl-c">// ⮕ Shape&#x3C;null></span>
</code></pre><h1 id="number"><a class="markdown-permalink" href="#number"><span class="icon icon-link"></span></a><code>number</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.number.html"><code>d.number</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html"><code>NumberShape</code><span class="external"></span></a> instance.</p><p>The shape that requires an input to be a number.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">number</span>();
<span class="pl-c">// ⮕ Shape&#x3C;number></span>
...
<span class="pl-c">// ⮕ 0</span>
</code></pre><p>Limit the allowed range:</p><pre><code class="language-ts"><span class="pl-c">// The number must be greater than 5 and less than or equal to 10</span>
...
<span class="pl-c">// ❌ ValidationError: type.number at /: Must be a number</span>
</code></pre><h1 id="object"><a class="markdown-permalink" href="#object"><span class="icon icon-link"></span></a><code>object</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.object.html"><code>d.object</code><span class="external"></span></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html"><code>ObjectShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be an object with a set of properties:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">object</span>({
  name: <span class="pl-smi">d</span>.<span class="pl-en">string</span>(),
  age: <span class="pl-smi">d</span>.<span class="pl-en">number</span>(),
});
<span class="pl-c">// ⮕ Shape&#x3C;{ name: string, age: number }></span>
...
<span class="pl-c">// Unknown keys are now preserved</span>
...
</code></pre><h2 id="key-relationships"><a class="markdown-permalink" href="#key-relationships"><span class="icon icon-link"></span></a>Key relationships</h2><p>Declare relationships between object keys using <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#allkeys"><code>allKeys</code><span class="external"></span></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#notallkeys"><code>notAllKeys</code><span class="external"></span></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#orkeys"><code>orKeys</code><span class="external"></span></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#xorkeys"><code>xorKeys</code><span class="external"></span></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#oxorkeys"><code>oxorKeys</code><span class="external"></span></a></p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>
  .<span class="pl-en">object</span>({
    foo: <span class="pl-smi">d</span>.<span class="pl-en">string</span>(),
    bar: <span class="pl-smi">d</span>.<span class="pl-en">number</span>(),
...
</code></pre><h1 id="promise"><a class="markdown-permalink" href="#promise"><span class="icon icon-link"></span></a><code>promise</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.promise.html"><code>d.promise</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.PromiseShape.html"><code>PromiseShape</code><span class="external"></span></a> instance.</p><p>The shape that checks that an input is an instance of <code>Promise</code>.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">promise</span>();
<span class="pl-c">// ⮕ Shape&#x3C;Promise&#x3C;any>></span>
...
</code></pre><h1 id="record"><a class="markdown-permalink" href="#record"><span class="icon icon-link"></span></a><code>record</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.record.html"><code>d.record</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.RecordShape.html"><code>RecordShape</code><span class="external"></span></a> instance.</p><p>Constrain keys and values of a dictionary-like object:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">record</span>(<span class="pl-smi">d</span>.<span class="pl-en">number</span>());
...
</code></pre><h1 id="set"><a class="markdown-permalink" href="#set"><span class="icon icon-link"></span></a><code>set</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.set.html"><code>d.set</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html"><code>SetShape</code><span class="external"></span></a> instance.</p><p>Constrains an input to be a <code>Set</code> instance:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-c1">set</span>(<span class="pl-smi">d</span>.<span class="pl-en">number</span>());
...
</code></pre><h1 id="string"><a class="markdown-permalink" href="#string"><span class="icon icon-link"></span></a><code>string</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.string.html"><code>d.string</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html"><code>StringShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be string.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">string</span>();
<span class="pl-c">// ⮕ Shape&#x3C;string></span>
...
</code></pre><h1 id="tuple"><a class="markdown-permalink" href="#tuple"><span class="icon icon-link"></span></a><code>tuple</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.tuple.html"><code>d.tuple</code><span class="external"></span></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html"><code>ArrayShape</code><span class="external"></span></a> instance.</p><p>Constrains a value to be a tuple where elements at particular positions have concrete types:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">tuple</span>([<span class="pl-smi">d</span>.<span class="pl-en">string</span>(), <span class="pl-smi">d</span>.<span class="pl-en">number</span>()]);
<span class="pl-c">// ⮕ Shape&#x3C;[string, number]></span>
...
<span class="pl-c">// ⮕ Shape&#x3C;[string], readonly [string]></span>
</code></pre><p>Tuples follow <a href="#coerce-to-an-array">array type coercion rules</a>.</p><h1 id="undefined"><a class="markdown-permalink" href="#undefined"><span class="icon icon-link"></span></a><code>undefined</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.undefined.html"><code>d.undefined</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.ConstShape.html"><code>ConstShape</code><span class="external"></span></a> instance.</p><p>The shape that requires an input to be <code>undefined</code>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">undefined</span>();
<span class="pl-c">// ⮕ Shape&#x3C;undefined></span>
</code></pre><h1 id="union-or"><a class="markdown-permalink" href="#union-or"><span class="icon icon-link"></span></a><code>union</code>, <code>or</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.union.html"><code>d.union</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.UnionShape.html"><code>UnionShape</code><span class="external"></span></a> instance.</p><p>A constraint that allows a value to be one of the given types:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">union</span>([<span class="pl-smi">d</span>.<span class="pl-en">string</span>(), <span class="pl-smi">d</span>.<span class="pl-en">number</span>()]);
<span class="pl-c">// ⮕ Shape&#x3C;string | number></span>
...
<span class="pl-c">// ⮕ Shape&#x3C;{ businessType: 'entrepreneur', name: string, age: number }></span>
</code></pre><p>We're going to use <code>businessType</code> property as the discriminator in our union. Now let's define a shape for a company:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">companyShape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">object</span>({
...
<span class="pl-c">// ⮕ Shape&#x3C;{ businessType: 'llc' | 'corporation' | 'partnership', headcount: number }></span>
...
</code></pre><p>This behaviour is applied to discriminated unions as well.</p><h1 id="unknown"><a class="markdown-permalink" href="#unknown"><span class="icon icon-link"></span></a><code>unknown</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.unknown.html"><code>d.unknown</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html"><code>Shape</code><span class="external"></span></a> instance.</p><p>An unconstrained value that is inferred as <code>unknown</code>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">unknown</span>();
<span class="pl-c">// ⮕ Shape&#x3C;unknown></span>
</code></pre><h1 id="void"><a class="markdown-permalink" href="#void"><span class="icon icon-link"></span></a><code>void</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.void.html"><code>d.void</code><span class="external"></span></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.ConstShape.html"><code>ConstShape</code><span class="external"></span></a> instance.</p><p>The shape that requires an input to be <code>undefined</code> that is typed as <code>void</code>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">void</span>();
...
<span class="pl-c">// ⮕ { '': [], name: ['Bill'], age: ['42'] }</span>
  ...
  .<span class="pl-en">strip</span>();
</code></pre><p><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#strip"><code>strip</code><span class="external"></span></a> removes all unknown keys from an object. It is used here to prevent unexpected arguments from being accessible inside the app. You may want to throw an error if unknown keys are detected or ignore them. Refer to <a href="#unknown-keys">Unknown keys</a> section to find out how this can be done.</p><p>Parse CLI arguments using <code>optionsShape</code> with enabled <a href="#type-coercion">type coercion</a>:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">options</span> <span class="pl-k">=</span> <span class="pl-smi">optionsShape</span>.<span class="pl-c1">parse</span>(<span class="pl-smi">args</span>);
<span class="pl-c">// ⮕ { name: 'Bill', age: 42 }</span>
</code></pre><h2 id="type-safe-localstorage"><a class="markdown-permalink" href="#type-safe-localstorage"><span class="icon icon-link"></span></a>Type-safe <code>localStorage</code></h2><p><a href="https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage"><code>localStorage</code><span class="external"></span></a> is a key-value storage which allows persistence of string keys and string values on the client. Let's write two functions that can read and write JSON objects from and to <code>localStorage</code> in a type-safe manner.</p><p>First lets define a shape of the data stored in the <code>localStorage</code>. In this example <code>localStorage</code> would allow only one key <code>'user'</code> that would correspond to an object with <code>name</code> and <code>age</code> properties:</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-c1">*</span> <span class="pl-k">as</span> <span class="pl-smi">d</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter<span class="pl-pds">'</span></span>;
...
</code></pre><p>Read more about <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#at"><code>Shape.at</code><span class="external"></span></a> method in the <a href="#nested-shapes">Nested shapes</a> section. The same approach can be taken to implement writes:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">setItem</span>&#x3C;<span class="pl-en">K</span> <span class="pl-k">extends</span> <span class="pl-k">keyof</span> <span class="pl-en">LocalStorageItems</span>>(<span class="pl-v">key</span><span class="pl-k">:</span> <span class="pl-en">K</span>, <span class="pl-v">value</span><span class="pl-k">:</span> <span class="pl-en">LocalStorageItems</span>[<span class="pl-en">K</span>])<span class="pl-k">:</span> <span class="pl-c1">void</span> {
  <span class="pl-k">const</span> <span class="pl-c1">valueShape</span> <span class="pl-k">=</span> <span class="pl-smi">localStorageItemsShape</span>.<span class="pl-en">at</span>(<span class="pl-smi">key</span>);
...
</code></pre><p>Note that we prevent writes of unknown keys as well as reads. Now, let's use those functions:</p><pre><code class="language-ts"><span class="pl-en">setItem</span>(<span class="pl-s"><span class="pl-pds">'</span>user<span class="pl-pds">'</span></span>, { name: <span class="pl-s"><span class="pl-pds">'</span>John<span class="pl-pds">'</span></span>, age: <span class="pl-c1">42</span> });
...
</code></pre><p>Then, create a <a href="#record"><code>d.record</code></a> shape that constrains keys and values of a dictionary-like object:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">record</span>(<span class="pl-smi">keysShape</span>, <span class="pl-smi">d</span>.<span class="pl-en">number</span>());
...
<span class="pl-c">// ❌ ValidationError: number.gte at /: Must be greater than 0</span>
</code></pre>`};function l(){return s.createElement(a,{logo:s.createElement(`div`,{style:o(r,n),className:i.Logo,title:`Doubter`}),readme:c})}export{l as default};