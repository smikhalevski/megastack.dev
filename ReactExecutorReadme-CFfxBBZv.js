...
</code></pre><p>Each executor has a unique key in the scope of the manager. Here we created the new executor with the key <code>'rooky'</code>. Managers create a new executor when you call <a href="https://smikhalevski.github.io/react-executor/classes/react-executor.ExecutorManager.html#getorcreate"><code>getOrCreate</code></a> with a new key. Each consequent call with that key returns the same executor.</p><p>If you want to retrieve an existing executor by its key and don't want to create a new executor if it doesn't exist, use <a href="https://smikhalevski.github.io/react-executor/classes/react-executor.ExecutorManager.html#get"><code>get</code></a>:</p><pre><code class="language-ts"><span class="pl-smi">manager</span>.<span class="pl-c1">get</span>(<span class="pl-s"><span class="pl-pds">'</span>bobby<span class="pl-pds">'</span></span>);
...
</code></pre><p>To override, how keys are serialized pass <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.ExecutorManagerOptions.html#keyserializer"><code>keySerializer</code></a> option to the <code>ExecutorManager</code> constructor. Key serializer is a function that receives the requested executor key and returns its serialized form. The returned serialized key form can be anything, a string, or an object.</p><p>If you're using objects as executor keys, then you may want to enable stable serialization (when keys are sorted alphabetically during serialization). In this case use any library that supports stable JSON serialization:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">stringify</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>json-marshal<span class="pl-pds">'</span></span>;
...
</code></pre><p>The executor keeps track of the <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#task">latest task</a> it has executed:</p><pre><code class="language-ts"><span class="pl-smi">rookyExecutor</span>.<span class="pl-smi">task</span>;
...
</code></pre><p>Executors always preserve the latest value and the latest reason. So even when the executor <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#ispending"><code>isPending</code></a>, you can access the previous value or failure reason. Use <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#isfulfilled"><code>isFulfilled</code></a> and <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#isrejected"><code>isRejected</code></a> to detect with what result the executor has settled the last time. An executor cannot be both fulfilled and rejected at the same time.</p><pre><code class="language-ts"><span class="pl-c">// Execute a new task</span>
...
</code></pre><h2 id="abort-a-task"><a class="markdown-permalink" href="#abort-a-task"><span class="icon icon-link"></span></a>Abort a task</h2><p>The promise returned by the <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#execute"><code>execute</code></a> method is <a href="https://smikhalevski.github.io/parallel-universe/classes/AbortablePromise.html">abortable</a> so the task can be prematurely aborted. Results of the aborted task are discarded:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">helloPromise</span> <span class="pl-k">=</span> <span class="pl-smi">rookyExecutor</span>.<span class="pl-en">execute</span>(<span class="pl-k">async</span> () <span class="pl-k">=></span> <span class="pl-s"><span class="pl-pds">'</span>Hello<span class="pl-pds">'</span></span>);
...
</code></pre><h2 id="retry-the-latest-task"><a class="markdown-permalink" href="#retry-the-latest-task"><span class="icon icon-link"></span></a>Retry the latest task</h2><p>To retry the <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#task">latest task</a>, use <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#retry"><code>retry</code></a>:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">planets</span> <span class="pl-k">=</span> [<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Venus<span class="pl-pds">'</span></span>];
...
<span class="pl-smi">executor</span>.<span class="pl-en">retry</span>();
</code></pre><h2 id="settle-an-executor"><a class="markdown-permalink" href="#settle-an-executor"><span class="icon icon-link"></span></a>Settle an executor</h2><p>While tasks are always handled in an asynchronous fashion, there are cases when an executor should be settled synchronously.</p><p>Executor can be synchronously fulfilled via <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#resolve"><code>resolve</code></a>:</p><pre><code class="language-ts"><span class="pl-smi">executor</span>.<span class="pl-c1">resolve</span>(<span class="pl-s"><span class="pl-pds">'</span>Venus<span class="pl-pds">'</span></span>);
...
<span class="pl-smi">executor</span>.<span class="pl-c1">value</span>;
<span class="pl-c">// ⮕ 'Venus'</span>
</code></pre><p>Or rejected via <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#reject"><code>reject</code></a>:</p><pre><code class="language-ts"><span class="pl-smi">executor</span>.<span class="pl-c1">reject</span>(<span class="pl-k">new</span> <span class="pl-c1">Error</span>(<span class="pl-s"><span class="pl-pds">'</span>Ooops!<span class="pl-pds">'</span></span>));
...
<span class="pl-smi">executor</span>.<span class="pl-c1">value</span>;
<span class="pl-c">// ⮕ 'Mars'</span>
</code></pre><h2 id="clear-an-executor"><a class="markdown-permalink" href="#clear-an-executor"><span class="icon icon-link"></span></a>Clear an executor</h2><p>After the executor becomes <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#issettled">settled</a>, it remains settled until it is cleared.</p><p>You can reset the executor back to its unsettled state using <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#clear"><code>clear</code></a>:</p><pre><code class="language-ts"><span class="pl-smi">executor</span>.<span class="pl-c1">clear</span>();
</code></pre><p>Clearing an executor removes the stored value and reason, but <em>doesn't</em> affect the pending task execution and preserves the <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#task">latest task</a> that was executed.</p><h1 id="events-and-lifecycle"><a class="markdown-permalink" href="#events-and-lifecycle"><span class="icon icon-link"></span></a>Events and lifecycle</h1><p>Executors publish various events when their state changes. To subscribe to executor events use the <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#subscribe"><code>subscribe</code></a> method:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">manager</span> <span class="pl-k">=</span> <span class="pl-k">new</span> <span class="pl-en">ExecutorManager</span>();
...
</code></pre><p>Both executors and managers may have multiple subscribers and each subscriber receives <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.ExecutorEvent.html">events</a> with following types:</p><dl><dt>attached</dt><dd><p>The executor was just created, plugins were applied to it, and it was attached to the manager. Read more about plugins in the <a href="#plugins">Plugins</a> section.</p></dd><dt>detached</dt><dd><p>The executor was just detached: it was removed from the manager and all of its subscribers were unsubscribed. Read more in the <a href="#detach-an-executor">Detach an executor</a> section.</p></dd><dt>activated</dt><dd><p>The executor was inactive and became active. This means that there are consumers that observe the state of the executor. Read more in the <a href="#activate-an-executor">Activate an executor</a> section.</p></dd><dt>deactivated</dt><dd><p>The executor was active and became inactive. This means that there are no consumers that observe the state of the executor. Read more in the <a href="#activate-an-executor">Activate an executor</a> section.</p></dd><dt>pending</dt><dd><p>The executor started <a href="#execute-a-task">a task execution</a>. You can find the latest task the executor handled in the <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#task"><code>Executor.task</code></a> property.</p></dd><dt>fulfilled</dt><dd><p>The executor was fulfilled with a value.</p></dd><dt>rejected</dt><dd><p>The executor was rejected with a reason.</p></dd><dt>aborted</dt><dd><p>The <a href="#abort-a-task">task was aborted</a>.</p><p>If executor is still <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#ispending">pending</a> when an <code>'aborted'</code> event is published then the currently pending task is being <a href="#replace-a-task">replaced</a> with a new task.</p><p>Calling <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#execute"><code>Executor.execute</code></a> when handling an abort event may lead to stack overflow. If you need to do this anyway, execute a new task from async context using <a href="https://developer.mozilla.org/en-US/docs/Web/API/queueMicrotask"><code>queueMicrotask</code></a> or a similar API.</p></dd><dt>cleared</dt><dd><p>The executor was cleared and now isn't settled.</p></dd><dt>invalidated</dt><dd><p>Results stored in an executor were <a href="#invalidate-results">invalidated</a>.</p></dd><dt>annotated</dt><dd><p><a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#annotations">Annotations</a> associated with the executor were patched.</p></dd><dt>plugin_configured</dt><dd><p>The configuration of the plugin associated with the executor was updated.</p></dd></dl><h2 id="activate-an-executor"><a class="markdown-permalink" href="#activate-an-executor"><span class="icon icon-link"></span></a>Activate an executor</h2><p>Executors have an <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#isactive">active</a> status that tells whether executor is actively used by a consumer.</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">deactivate</span> <span class="pl-k">=</span> <span class="pl-smi">executor</span>.<span class="pl-en">activate</span>();
...
<span class="pl-smi">executor</span>.<span class="pl-smi">isInvalidated</span>;
<span class="pl-c">// ⮕ false</span>
</code></pre><p>By default, invalidating an executor has no effect except marking it as <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#isinvalidated">invalidated</a>.</p><h2 id="detach-an-executor"><a class="markdown-permalink" href="#detach-an-executor"><span class="icon icon-link"></span></a>Detach an executor</h2><p>By default, executors that a manager has created are preserved indefinitely and are always available though <a href="https://smikhalevski.github.io/react-executor/classes/react-executor.ExecutorManager.html#get"><code>get</code></a>. This isn't always optimal, and you may want to detach an executor when it isn't needed anymore. Use <a href="https://smikhalevski.github.io/react-executor/classes/react-executor.ExecutorManager.html#detach"><code>detach</code></a> in such case:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">=</span> <span class="pl-smi">manager</span>.<span class="pl-en">getOrCreate</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>);
...
</code></pre><p>To apply a plugin, pass it to the <a href="https://smikhalevski.github.io/react-executor/classes/react-executor.ExecutorManager.html#getorcreate"><code>ExecutorManager.getOrCreate</code></a> or to the <a href="https://smikhalevski.github.io/react-executor/functions/react-executor.useExecutor.html"><code>useExecutor</code></a> hook:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">=</span> <span class="pl-smi">manager</span>.<span class="pl-en">getOrCreate</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-c1">undefined</span>, [<span class="pl-smi">detachPlugin</span>]);
...
</code></pre><h2 id="resolveby"><a class="markdown-permalink" href="#resolveby"><span class="icon icon-link"></span></a><code>resolveBy</code></h2><p><a href="#settle-an-executor">Resolves the executor</a> with values pushed by an <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Observable.html"><code>Observable</code></a>.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">Observable</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="react-integration"><a class="markdown-permalink" href="#react-integration"><span class="icon icon-link"></span></a>React integration</h1><p>In the basic scenario, to use executors in your React app, you don't need any additional configuration, just use the <a href="https://smikhalevski.github.io/react-executor/functions/react-executor.useExecutor.html"><code>useExecutor</code></a> hook right away:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">useExecutor</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor<span class="pl-pds">'</span></span>;
...
</code></pre><p>Every time the executor's state is changed, the component is re-rendered. The executor returned from the hook is <a href="#activate-an-executor">activated</a> after mount and deactivated on unmount.</p><p>The hook has the exact same signature as the <a href="https://smikhalevski.github.io/react-executor/classes/react-executor.ExecutorManager.html#getorcreate"><code>ExecutorManager.getOrCreate</code></a> method, described in the <a href="#introduction">Introduction</a> section.</p><div class="markdown-alert markdown-alert-tip"><p class="markdown-alert-title"><svg class="octicon octicon-light-bulb mr-2" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="M8 1.5c-2.363 0-4 1.69-4 3.75 0 .984.424 1.625.984 2.304l.214.253c.223.264.47.556.673.848.284.411.537.896.621 1.49a.75.75 0 0 1-1.484.211c-.04-.282-.163-.547-.37-.847a8.456 8.456 0 0 0-.542-.68c-.084-.1-.173-.205-.268-.32C3.201 7.75 2.5 6.766 2.5 5.25 2.5 2.31 4.863 0 8 0s5.5 2.31 5.5 5.25c0 1.516-.701 2.5-1.328 3.259-.095.115-.184.22-.268.319-.207.245-.383.453-.541.681-.208.3-.33.565-.37.847a.751.751 0 0 1-1.485-.212c.084-.593.337-1.078.621-1.489.203-.292.45-.584.673-.848.075-.088.147-.173.213-.253.561-.679.985-1.32.985-2.304 0-2.06-1.637-3.75-4-3.75ZM5.75 12h4.5a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1 0-1.5ZM6 15.25a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 0 1.5h-2.5a.75.75 0 0 1-.75-.75Z"></path></svg>Tip</p><p>Check out the live example of <a href="https://stackblitz.com/edit/react-executor-todo-app?file=README.md">the TODO app</a> that employs React Executor.</p></div><p>You can use executors both inside and outside the rendering process. To do this, provide a custom <a href="https://smikhalevski.github.io/react-executor/classes/react-executor.ExecutorManager.html"><code>ExecutorManager</code></a> through the context:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">ExecutorManager</span>, <span class="pl-smi">ExecutorManagerProvider</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor<span class="pl-pds">'</span></span>;
...
</code></pre><p>If you want to have access to an executor in a component, but don't want to re-render the component when the executor's state is changed, use <a href="https://smikhalevski.github.io/react-executor/functions/react-executor.useExecutorManager.html"><code>useExecutorManager</code></a> hook:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">accountExecutor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutorManager</span>().<span class="pl-en">getOrCreate</span>(<span class="pl-s"><span class="pl-pds">'</span>account<span class="pl-pds">'</span></span>);
...
</code></pre><h2 id="suspense"><a class="markdown-permalink" href="#suspense"><span class="icon icon-link"></span></a>Suspense</h2><p>Executors support fetch-as-you-render approach and can be integrated with React Suspense. To facilitate the rendering suspension, use the <a href="https://smikhalevski.github.io/react-executor/functions/react-executor.useExecutorSuspense.html"><code>useExecutorSuspense</code></a> hook:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">useExecutor</span>, <span class="pl-smi">useExecutorSuspense</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor<span class="pl-pds">'</span></span>;
...
</code></pre><p>By default, invalidating an executor has no additional effect. If you want to <a href="#retry-the-latest-task">retry the latest task</a> that each executor has executed, use <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Executor.html#retry"><code>retry</code></a>:</p><pre><code class="language-ts"><span class="pl-k">for</span> (<span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">of</span> <span class="pl-smi">manager</span>) {
  <span class="pl-smi">executor</span>.<span class="pl-en">retry</span>();
}
...
</code></pre><h2 id="global-loading-indicator"><a class="markdown-permalink" href="#global-loading-indicator"><span class="icon icon-link"></span></a>Global loading indicator</h2><p>To detect a global pending state we can rely on events published by an <a href="https://smikhalevski.github.io/react-executor/classes/react-executor.ExecutorManager.html"><code>ExecutorManager</code></a>:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">useGlobalPending</span>(<span class="pl-en">predicate</span> <span class="pl-k">=</span> (<span class="pl-v">executor</span><span class="pl-k">:</span> <span class="pl-en">Executor</span>) <span class="pl-k">=></span> <span class="pl-c1">true</span>)<span class="pl-k">:</span> <span class="pl-c1">boolean</span> {
