...
</code></pre><p>Install web dependencies:</p><pre><code class="language-shell">npm install racehorse
...
</code></pre><p>Create an <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-event-bridge/index.html"><code>EventBridge</code><span class="external"></span></a> instance that would be responsible for event marshalling:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.EventBridge</span>
...
</code></pre><p>Racehorse uses a <a href="https://greenrobot.org/eventbus">Greenrobot EventBus<span class="external"></span></a> to deliver events to subscribers, so the bridge must be registered in the event bus:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.greenrobot.eventbus.EventBus</span>
...
</code></pre><p>Note that <code>ShowToastEvent</code> implements the <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-web-event/index.html"><code>WebEvent</code><span class="external"></span></a> marker interface. This is the baseline requirement that events must conform to in order to support marshalling from the web app to Android.</p><p>Now let's add an event subscriber that would receive incoming <code>ShowToastEvent</code> and display a toast:</p><pre><code class="language-kotlin"><span class="pl-k">package</span> <span class="pl-en">com.example</span>
...
</code></pre><p>Register <code>ToastPlugin</code> in the event bus to enable event subscriptions:</p><pre><code class="language-kotlin"><span class="pl-en">EventBus</span>.getDefault().register(<span class="pl-en">ToastPlugin</span>(activity))
...
</code></pre><p>The last step is to load the web app into the WebView. You can do this in any way that fits your needs — Racehorse doesn't restrict this process in any way. For example, if your web app is running on your local machine on port 1234, you can load the web app in the WebView using this snippet:</p><pre><code class="language-kotlin">webView.loadUrl(<span class="pl-s"><span class="pl-pds">"</span>https://10.0.2.2:1234<span class="pl-pds">"</span></span>)
</code></pre><h1 id="example-app"><a class="markdown-permalink" href="#example-app"><span class="icon icon-link"></span></a>Example app</h1><p>The example app consists of two parts: <a href="https://github.com/smikhalevski/racehorse/tree/master/web/example">the web app<span class="external"></span></a> and <a href="https://github.com/smikhalevski/racehorse/tree/master/android/example">the Android app<span class="external"></span></a>. To launch the app in the emulator, follow the steps below.</p><p>Clone this repo:</p><pre><code class="language-shell">git clone git@github.com:smikhalevski/racehorse.git
...
npm start
</code></pre><p>Open <code>&#x3C;racehorse>/android</code> in Android Studio and run the <code>example</code> app.</p><h1 id="request-response-event-chains"><a class="markdown-permalink" href="#request-response-event-chains"><span class="icon icon-link"></span></a>Request-response event chains</h1><p>In the <a href="#overview">Overview</a> section we used an event that extends the <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-web-event/index.html"><code>WebEvent</code><span class="external"></span></a> interface. Such events don't imply a response. To create a request-response chain, at least two events are required:</p><pre><code class="language-kotlin"><span class="pl-k">package</span> <span class="pl-en">com.example</span>
...
</code></pre><p>Request and response events are instances of <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-chainable-event/index.html"><code>ChainableEvent</code><span class="external"></span></a>. Events in the chain share the same <code>requestId</code>. When a <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-response-event/index.html"><code>ResponseEvent</code><span class="external"></span></a> is posted to the event bus, it is marshalled to the web app and resolves a promise returned from <a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.EventBridge.html#requestasync"><code>eventBridge.requestAsync</code><span class="external"></span></a>:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">eventBridge</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
</code></pre><p>If an exception is thrown in <code>DeviceModelPlugin.onGetDeviceModel</code>, the promise is <em>rejected</em> with an <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error"><code>Error</code><span class="external"></span></a> instance.</p><h2 id="synchronous-requests"><a class="markdown-permalink" href="#synchronous-requests"><span class="icon icon-link"></span></a>Synchronous requests</h2><p>If all events in the event chain are handled on <a href="https://greenrobot.org/eventbus/documentation/delivery-threads-threadmode/">the posting thread<span class="external"></span></a> on the Android side, then a request can be handled synchronously on the web side. In the <code>DeviceModelPlugin</code> example, <code>onGetDeviceModel</code> is called on the posting thread since we didn't specify a thread mode for the <a href="https://github.com/greenrobot/EventBus#eventbus-in-3-steps"><code>@Subscribe</code><span class="external"></span></a> annotation. This allows the web to perform a synchronous request:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">eventBridge</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="event-subscriptions"><a class="markdown-permalink" href="#event-subscriptions"><span class="icon icon-link"></span></a>Event subscriptions</h1><p>While the web app can post a request event to Android, it is frequently required that Android post an event to the web app without an explicit request. This can be achieved using subscriptions.</p><p>Let's define an event that Android can post to the web:</p><pre><code class="language-kotlin"><span class="pl-k">package</span> <span class="pl-en">com.example</span>
...
</code></pre><p>To subscribe to an event of a given type, you can use a shortcut:</p><pre><code class="language-js"><span class="pl-k">import</span> { <span class="pl-smi">eventBridge</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
</code></pre><p>If you have <a href="#overview">an <code>EventBridge</code> registered</a> in the event bus, then you can post a <code>BatteryLowEvent</code> from anywhere in your Android app and it will be delivered to a subscriber in the web app:</p><pre><code class="language-kotlin"><span class="pl-en">EventBus</span>.getDefault().post(<span class="pl-en">BatteryLowEvent</span>())
</code></pre><h1 id="webview-events"><a class="markdown-permalink" href="#webview-events"><span class="icon icon-link"></span></a>WebView events</h1><p>Racehorse provides clients for the WebView which post WebView-related events to the event bus, so you can subscribe to them in your plugins. To init clients, set them on the WebView instance:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.webview.RacehorseWebChromeClient</span>
...
</code></pre><p>Now you can subscribe to <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse.webview/index.html">all events that a WebView instance posts<span class="external"></span></a>:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.greenrobot.eventbus.Subscribe</span>
...
</code></pre><h1 id="check-supported-events"><a class="markdown-permalink" href="#check-supported-events"><span class="icon icon-link"></span></a>Check supported events</h1><p>The web app can check that an event is supported by the Android binary. For example, to check that the app supports Google Pay card tokenization, you can use:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">eventBridge</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="proguard"><a class="markdown-permalink" href="#proguard"><span class="icon icon-link"></span></a>Proguard</h1><p><code>org.racehorse:racehorse</code> is an Android library (AAR) that provides its own <a href="https://github.com/smikhalevski/racehorse/tree/master/android/racehorse/proguard-rules.pro">proguard rules<span class="external"></span></a>, so no additional action is needed. Proguard rules prevent obfuscation of events and related classes available in Racehorse.</p><p>For example, this class and its members won't be minified:</p><pre><code class="language-kotlin"><span class="pl-k">class</span> <span class="pl-en">ShowToastEvent</span>(<span class="pl-k">val</span> <span class="pl-smi">message</span><span class="pl-k">:</span> <span class="pl-c1">String</span>) : WebEvent
</code></pre><h1 id="activity-plugin"><a class="markdown-permalink" href="#activity-plugin"><span class="icon icon-link"></span></a>Activity plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.ActivityManager.html"><code>ActivityManager</code><span class="external"></span></a> starts activities and provides info about the activity that renders the WebView.</p><p>Add the Lifecycle dependency to your Android app:</p><pre><code class="language-kotlin">dependencies {
...
</code></pre><p>Start a new activity. For example, here's how to open the Settings app and navigate the user to notification settings:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">activityManager</span>, <span class="pl-smi">Intent</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
</code></pre><p>If you are using React, refer to the <a href="https://smikhalevski.github.io/racehorse/functions/_racehorse_react.useActivityState.html"><code>useActivityState</code><span class="external"></span></a> hook, which re-renders a component when the activity state changes.</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">useActivityState</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@racehorse/react<span class="pl-pds">'</span></span>;

<span class="pl-k">const</span> <span class="pl-c1">state</span> <span class="pl-k">=</span> <span class="pl-en">useActivityState</span>();
<span class="pl-c">// ⮕ ActivityState.BACKGROUND</span>
</code></pre><h1 id="asset-loader-plugin"><a class="markdown-permalink" href="#asset-loader-plugin"><span class="icon icon-link"></span></a>Asset loader plugin</h1><p>The asset loader plugin requires <a href="#webview-events">WebView events</a> to be enabled.</p><p>Add the WebKit dependency:</p><pre><code class="language-kotlin">dependencies {
    implementation(<span class="pl-s"><span class="pl-pds">"</span>androidx.webkit:webkit:1.11.0<span class="pl-pds">"</span></span>)
}
</code></pre><p>Load static assets from a directory on the device when a particular URL is requested in the WebView:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">androidx.webkit.WebViewAssetLoader</span>
...
</code></pre><p>During development, if you're running a server on localhost, use <code>ProxyPathHandler</code> to serve content to the WebView:</p><pre><code class="language-kotlin"><span class="pl-en">AssetLoaderPlugin</span>(activity).<span class="pl-c1">apply</span> {
    registerAssetLoader(
        <span class="pl-s"><span class="pl-pds">"</span>https://example.com<span class="pl-pds">"</span></span>,
...
</code></pre><p><code>AssetLoaderPlugin</code> will open a URL in an external browser app if it isn't handled by any of the registered asset loaders. Since in the example above only <code>https://example.com</code> is handled by the asset loader, all other URLs are opened externally:</p><pre><code class="language-js"><span class="pl-c">// This would open a browser app and load google.com</span>
...
</code></pre><h1 id="biometric-plugin"><a class="markdown-permalink" href="#biometric-plugin"><span class="icon icon-link"></span></a>Biometric plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.BiometricManager.html"><code>BiometricManager</code><span class="external"></span></a> provides the status of biometric support and allows enrolling for biometric auth.</p><p>Add the <a href="https://developer.android.com/jetpack/androidx/releases/biometric#declaring_dependencies">Biometric<span class="external"></span></a> dependency to your Android app:</p><pre><code class="language-kotlin">dependencies {
...
</code></pre><h1 id="biometric-encrypted-storage-plugin"><a class="markdown-permalink" href="#biometric-encrypted-storage-plugin"><span class="icon icon-link"></span></a>Biometric encrypted storage plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.BiometricEncryptedStorageManager.html"><code>BiometricEncryptedStorageManager</code><span class="external"></span></a> enables file-based persistence of biometric-protected data.</p><p>Add the <a href="https://developer.android.com/jetpack/androidx/releases/biometric#declaring_dependencies">Biometric<span class="external"></span></a> dependency to your Android app:</p><pre><code class="language-kotlin">dependencies {
    implementation(<span class="pl-s"><span class="pl-pds">"</span>androidx.biometric:biometric:1.2.0-alpha05<span class="pl-pds">"</span></span>)
}
...
</code></pre><p>To allow device credential authentication, provide an <a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.BiometricConfig.html#authenticationvalidityduration"><code>authenticationValidityDuration</code><span class="external"></span></a> value greater than or equal to 0:</p><pre><code class="language-ts"><span class="pl-k">await</span> <span class="pl-smi">biometricEncryptedStorageManager</span>.<span class="pl-c1">set</span>(<span class="pl-s"><span class="pl-pds">'</span>foo<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>bar<span class="pl-pds">'</span></span>, {
...
</code></pre><p>If a user enrolls biometric auth (for example, updates fingerprints stored on the device), all secret keys used by the biometric-encrypted storage are invalidated and values become inaccessible.</p><pre><code class="language-js"><span class="pl-k">if</span> (<span class="pl-smi">biometricEncryptedStorageManager</span>.<span class="pl-c1">has</span>(key)) {
  <span class="pl-c">// Storage contains the key</span>

  <span class="pl-smi">biometricEncryptedStorageManager</span>.<span class="pl-c1">get</span>(key).<span class="pl-c1">then</span>(<span class="pl-smi">value</span> <span class="pl-k">=></span> {
    <span class="pl-k">if</span> (value <span class="pl-k">!==</span> <span class="pl-c1">null</span>) {
      ...
      <span class="pl-c">// User authentication failed, or key was invalidated</span>
    }
  });
}
</code></pre><p>If the user has not enrolled for biometric authentication, <code>set()</code> returns <code>false</code>. Check the enrollment status via the <a href="#biometric-plugin">Biometric plugin</a> before using the store:</p><pre><code class="language-ts"><span class="pl-k">if</span> (<span class="pl-smi">biometricManager</span>.<span class="pl-en">getBiometricStatus</span>([<span class="pl-smi">BiometricAuthenticator</span>.<span class="pl-c1">BIOMETRIC_STRONG</span>]) <span class="pl-k">===</span> <span class="pl-smi">BiometricStatus</span>.<span class="pl-c1">SUPPORTED</span>) {
  <span class="pl-k">await</span> <span class="pl-smi">biometricEncryptedStorageManager</span>.<span class="pl-c1">set</span>(<span class="pl-s"><span class="pl-pds">'</span>foo<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>bar<span class="pl-pds">'</span></span>);
}
</code></pre><h1 id="contacts-plugin"><a class="markdown-permalink" href="#contacts-plugin"><span class="icon icon-link"></span></a>Contacts plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.ContactsManager.html"><code>ContactsManager</code><span class="external"></span></a> provides access to contacts stored on the device.</p><p>Add the contacts permission to the app manifest:</p><pre><code class="language-xml">&#x3C;<span class="pl-ent">uses-permission</span> <span class="pl-e">android:name</span>=<span class="pl-s"><span class="pl-pds">"</span>android.permission.READ_CONTACTS<span class="pl-pds">"</span></span>/>
...
</code></pre><p>Ask a user to pick a contact or get a contact by its ID:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">contactsManager</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="deep-link-plugin"><a class="markdown-permalink" href="#deep-link-plugin"><span class="icon icon-link"></span></a>Deep link plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.DeepLinkManager.html"><code>DeepLinkManager</code><span class="external"></span></a> provides access to deep links inside your web app.</p><p>Initialize the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.DeepLinkPlugin</span>

<span class="pl-en">EventBus</span>.getDefault().register(<span class="pl-en">DeepLinkPlugin</span>())
</code></pre><p>Override <a href="https://developer.android.com/reference/android/app/Activity#onNewIntent(android.content.Intent)"><code>onNewIntent</code></a> in the main activity of your app and post the deep link event:</p><pre><code class="language-kotlin"><span class="pl-k">override</span> <span class="pl-k">fun</span> <span class="pl-en">onNewIntent</span>(<span class="pl-smi">intent</span><span class="pl-k">:</span> <span class="pl-en">Intent</span>) {
...
</code></pre><h1 id="device-plugin"><a class="markdown-permalink" href="#device-plugin"><span class="icon icon-link"></span></a>Device plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.DeviceManager.html"><code>DeviceManager</code><span class="external"></span></a> provides access to various device settings.</p><p>Add the compat library dependency, which is used for window insets acquisition:</p><pre><code class="language-kotlin">dependencies {
...
</code></pre><h1 id="download-plugin"><a class="markdown-permalink" href="#download-plugin"><span class="icon icon-link"></span></a>Download plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.DownloadManager.html"><code>DownloadManager</code><span class="external"></span></a> allows starting and monitoring file downloads.</p><p>Initialize the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.DownloadPlugin</span>
...
<span class="pl-smi">downloadManager</span>.<span class="pl-en">addDownload</span>(<span class="pl-s"><span class="pl-pds">'</span>http://example.com/my.zip<span class="pl-pds">'</span></span>).<span class="pl-c1">then</span>(<span class="pl-v">id</span> <span class="pl-k">=></span> {
  ...
  <span class="pl-c">// ⮕ Download { id: 1, status: 4, uri: 'http://example.com/my.zip' }</span>
});

<span class="pl-smi">downloadManager</span>.<span class="pl-en">getAllDownloads</span>();
</code></pre><p>A <a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.Download.html"><code>Download</code><span class="external"></span></a> instance carries the download status, progress, and file details.</p><p>A storage permission must be added to support Android devices with API level &#x3C;= 29:</p><pre><code class="language-xml">&#x3C;<span class="pl-ent">uses-permission</span>
...
</code></pre><h2 id="android-29-support"><a class="markdown-permalink" href="#android-29-support"><span class="icon icon-link"></span></a>Android 29 support</h2><p>On Android 29, a <code>SecurityException</code> is thrown when calling the deprecated method <a href="https://developer.android.com/reference/android/app/DownloadManager#addCompletedDownload(java.lang.String,%20java.lang.String,%20boolean,%20java.lang.String,%20java.lang.String,%20long,%20boolean)"><code>DownloadManager.addCompletedDownload</code></a> if the <code>android.permission.WRITE_EXTERNAL_STORAGE</code> permission isn't granted. This method is used by Racehorse to populate the list of previous downloads when a data URI is downloaded. To fix this exception, <a href="https://developer.android.com/training/data-storage/use-cases#opt-out-in-production-app">the legacy external storage model<span class="external"></span></a> must be enabled in the Android manifest for API level 29.</p><p>Create a resource file for default config values at <code>src/main/res/values/config.xml</code>:</p><pre><code class="language-xml">&#x3C;<span class="pl-ent">resources</span>>
...
&#x3C;/<span class="pl-ent">resources</span>>
</code></pre><p>Create a resource file specific to API level 29 at <code>src/main/res/values-v29/config.xml</code>:</p><pre><code class="language-xml">&#x3C;<span class="pl-ent">resources</span>>
...
&#x3C;/<span class="pl-ent">resources</span>>
</code></pre><p>Configure the legacy external storage setting in the Android manifest file:</p><pre><code class="language-xml">&#x3C;<span class="pl-ent">application</span>
...
/>
</code></pre><h2 id="downloadable-links"><a class="markdown-permalink" href="#downloadable-links"><span class="icon icon-link"></span></a>Downloadable links</h2><p>Downloadable links have a <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a"><code>download</code><span class="external"></span></a> attribute:</p><pre><code class="language-html">&#x3C;<span class="pl-ent">a</span>
...
</code></pre><p>Initialize the <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-download-plugin/index.html"><code>DownloadPlugin</code><span class="external"></span></a> as described in the previous section, and add a Racehorse listener to enable automatic handling of downloadable links:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.webview.RacehorseDownloadListener</span>

webView.setDownloadListener(<span class="pl-en">RacehorseDownloadListener</span>())
</code></pre><h1 id="encrypted-storage-plugin"><a class="markdown-permalink" href="#encrypted-storage-plugin"><span class="icon icon-link"></span></a>Encrypted storage plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.EncryptedStorageManager.html"><code>EncryptedStorageManager</code><span class="external"></span></a> enables file-based persistence of password-protected data.</p><p>Initialize the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.EncryptedStoragePlugin</span>
...
<span class="pl-c">// ⮕ 'bar'</span>
</code></pre><h1 id="evergreen-plugin"><a class="markdown-permalink" href="#evergreen-plugin"><span class="icon icon-link"></span></a>Evergreen plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.EvergreenManager.html"><code>EvergreenManager</code><span class="external"></span></a> provides a way to update your app using an archive downloadable from your server.</p><p>You can find an extensive demo of evergreen plugin usage <a href="#example-app">in the example app</a>.</p><p>Init the plugin and start the update download process:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">android.os.Bundle</span>
...
</code></pre><p>The snippet above downloads <code>bundle.zip</code>, unpacks it, and stores the assets in the <code>&#x3C;filesDir>/app</code> directory. These assets are labeled as version 1.0.0. During future app launches, the plugin will notice it already has the assets for version 1.0.0 and will skip the download. If the version changes, the update bundle will be downloaded again.</p><p>After the update is downloaded, a <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse.evergreen/-bundle-ready-event/index.html"><code>BundleReadyEvent</code><span class="external"></span></a> event is posted. You can use the <a href="#asset-loader-plugin"><code>AssetLoaderPlugin</code></a> to load resources provided by the evergreen plugin:</p><pre><code class="language-kotlin">@Subscribe(threadMode <span class="pl-k">=</span> <span class="pl-en">ThreadMode</span>.<span class="pl-en">MAIN</span>)
...
</code></pre><h1 id="facebook-login-plugin"><a class="markdown-permalink" href="#facebook-login-plugin"><span class="icon icon-link"></span></a>Facebook Login plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.FacebookLoginManager.html"><code>FacebookLoginManager</code><span class="external"></span></a> enables Facebook Login support.</p><p>Go to <a href="https://developers.facebook.com/docs/facebook-login/android/">developers.facebook.com<span class="external"></span></a>, register your app, and add the required dependencies and configurations.</p><p>Initialize the Facebook SDK and register the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">com.facebook.FacebookSdk</span>
...
</code></pre><p>Request sign-in from the web app loaded into the WebView:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">facebookLoginManager</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
<span class="pl-smi">facebookLoginManager</span>.<span class="pl-en">logIn</span>().<span class="pl-c1">then</span>(<span class="pl-v">accessToken</span> <span class="pl-k">=></span> {
  <span class="pl-c">// The accessToken is non-null if log-in succeeded</span>
});
</code></pre><h1 id="facebook-share-plugin"><a class="markdown-permalink" href="#facebook-share-plugin"><span class="icon icon-link"></span></a>Facebook Share plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.FacebookShareManager.html"><code>FacebookShareManager</code><span class="external"></span></a> enables Facebook social sharing.</p><p>Go to <a href="https://developers.facebook.com/docs/facebook-login/android/">developers.facebook.com<span class="external"></span></a>, register your app, and add the required dependencies and configurations.</p><p>Initialize the Facebook SDK and register the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">com.facebook.FacebookSdk</span>
...
</code></pre><p>Trigger the Facebook social sharing flow:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">facebookShareManager</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="file-chooser-plugin"><a class="markdown-permalink" href="#file-chooser-plugin"><span class="icon icon-link"></span></a>File chooser plugin</h1><p>The file chooser plugin requires <a href="#webview-events">WebView events</a> to be enabled. This plugin enables file inputs in the web app.</p><p>For example, if you have a file input:</p><pre><code class="language-html">&#x3C;<span class="pl-ent">input</span> <span class="pl-e">type</span>=<span class="pl-s"><span class="pl-pds">"</span>file<span class="pl-pds">"</span></span> />
...
</code></pre><p>If you don't need camera support for file inputs, the plugin doesn't require any additional configuration.</p><h2 id="enabling-camera-capture"><a class="markdown-permalink" href="#enabling-camera-capture"><span class="icon icon-link"></span></a>Enabling camera capture</h2><p>Camera capture requires a temporary file storage location to write captured files to.</p><p>Declare a provider in your app manifest:</p><pre><code class="language-xml">&#x3C;<span class="pl-ent">manifest</span>>
...
&#x3C;/<span class="pl-ent">paths</span>>
</code></pre><p>Initialize the plugin in your Android app, providing the authority of the provider you created and the path defined in the descriptor:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.FileChooserPlugin</span>
...
</code></pre><h2 id="serving-local-files"><a class="markdown-permalink" href="#serving-local-files"><span class="icon icon-link"></span></a>Serving local files</h2><p>To load an arbitrary file from the WebView, use <a href="https://smikhalevski.github.io/racehorse/classes/racehorse.File.html#localurl"><code>localUrl</code><span class="external"></span></a>:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">contactsManager</span>, <span class="pl-smi">fsManager</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
<span class="pl-c">// ⮕ 'https://racehorse.local/fs?uri=…'</span>
...
</code></pre><h1 id="google-pay-plugin"><a class="markdown-permalink" href="#google-pay-plugin"><span class="icon icon-link"></span></a>Google Pay plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.GooglePayManager.html"><code>GooglePayManager</code><span class="external"></span></a> enables <a href="https://developers.google.com/pay/issuers/apis/push-provisioning/android">Android Push Provisioning<span class="external"></span></a> support.</p><p><a href="https://developers.google.com/pay/issuers/apis/push-provisioning/android/setup">Set up the development environment<span class="external"></span></a> so the TapAndPay SDK is available in your app.</p><p>Initialize the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.GooglePayPlugin</span>
...
</code></pre><p>To open the wallet app and reveal the tokenized card:</p><pre><code class="language-ts"><span class="pl-k">async</span> <span class="pl-k">function</span> <span class="pl-en">revealCard</span>(<span class="pl-v">lastFour</span><span class="pl-k">:</span> <span class="pl-c1">string</span>)<span class="pl-k">:</span> <span class="pl-en">Promise</span>&#x3C;<span class="pl-c1">boolean</span>> {
  <span class="pl-k">const</span> <span class="pl-c1">tokenInfo</span> <span class="pl-k">=</span> <span class="pl-k">await</span> <span class="pl-en">getTokenInfo</span>(<span class="pl-smi">lastFour</span>);
...
</code></pre><h1 id="google-sign-in-plugin"><a class="markdown-permalink" href="#google-sign-in-plugin"><span class="icon icon-link"></span></a>Google Sign-In plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.GoogleSignInManager.html"><code>GoogleSignInManager</code><span class="external"></span></a> enables Google Sign-In support.</p><p>Go to <a href="https://console.firebase.google.com">console.firebase.google.com<span class="external"></span></a>, set up a new project, and configure an Android app following all instructions. Use the <code>applicationId</code> of your app and the SHA-1 used for app signing. You can use Gradle to retrieve SHA-1:</p><pre><code class="language-shell">./gradlew signingReport
</code></pre><p>Go to <a href="https://console.cloud.google.com/apis/credentials">Google Cloud Console<span class="external"></span></a> for your project and add an OAuth client ID for Android.</p><p>Add the Google Sign-In SDK dependencies to your Android app:</p><pre><code class="language-kotlin">dependencies {
...
</code></pre><p>Request sign-in from the web app loaded into a WebView:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">googleSignInManager</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
<span class="pl-smi">googleSignInManager</span>.<span class="pl-en">signIn</span>().<span class="pl-c1">then</span>(<span class="pl-v">account</span> <span class="pl-k">=></span> {
  <span class="pl-c">// The account is non-null if sign-in succeeded</span>
});
</code></pre><h1 id="https-plugin"><a class="markdown-permalink" href="#https-plugin"><span class="icon icon-link"></span></a>HTTPS plugin</h1><p>The HTTPS plugin requires <a href="#webview-events">WebView events</a> to be enabled. It forces the WebView to ignore certificate issues.</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.HttpsPlugin</span>

<span class="pl-en">EventBus</span>.getDefault().register(<span class="pl-en">HttpsPlugin</span>())
</code></pre><h1 id="keyboard-plugin"><a class="markdown-permalink" href="#keyboard-plugin"><span class="icon icon-link"></span></a>Keyboard plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.KeyboardManager.html"><code>KeyboardManager</code><span class="external"></span></a> toggles the software keyboard and notifies about keyboard animations.</p><p>Initialize the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.KeyboardPlugin</span>
...
</code></pre><p>Synchronously read the keyboard height, or show/hide the keyboard:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">keyboardManager</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
</code></pre><p>Subscribe to the keyboard manager to receive notifications when a keyboard animation starts:</p><pre><code class="language-ts"><span class="pl-smi">keyboardManager</span>.<span class="pl-en">subscribe</span>(<span class="pl-v">animation</span> <span class="pl-k">=></span> {
  <span class="pl-c">// Handle the started animation here.</span>
});
</code></pre><p>If you are using React, use the <a href="https://smikhalevski.github.io/racehorse/functions/_racehorse_react.useKeyboardAnimation.html"><code>useKeyboardAnimation</code><span class="external"></span></a> hook to subscribe to keyboard animations from a component:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">useKeyboardAnimation</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@racehorse/react<span class="pl-pds">'</span></span>;
...
<span class="pl-en">useKeyboardAnimation</span>((<span class="pl-v">animation</span>, <span class="pl-v">signal</span>) <span class="pl-k">=></span> {
  <span class="pl-c">// Signal is aborted if the animation is cancelled.</span>
});
</code></pre><p>Use <a href="https://smikhalevski.github.io/racehorse/functions/racehorse.runAnimation.html"><code>runAnimation</code><span class="external"></span></a> to run the animation. For example, if your <a href="https://developer.android.com/develop/ui/views/layout/edge-to-edge">app is rendered edge-to-edge<span class="external"></span></a>, you can animate the bottom padding to compensate for the height of the keyboard.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">useKeyboardAnimation</span>, <span class="pl-smi">runAnimation</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@racehorse/react<span class="pl-pds">'</span></span>;
...
</code></pre><p>You may also want to scroll the window to prevent the focused element from being obscured by the keyboard. Use <a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.scrollToElement.html"><code>scrollToElement</code><span class="external"></span></a> to animate scrolling in sync with the keyboard animation:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">useKeyboardAnimation</span>, <span class="pl-smi">scrollToElement</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@racehorse/react<span class="pl-pds">'</span></span>;
...
<span class="pl-en">useKeyboardAnimation</span>((<span class="pl-v">animation</span>, <span class="pl-v">signal</span>) <span class="pl-k">=></span> {
  ...
  <span class="pl-en">scrollToElement</span>(<span class="pl-c1">document</span>.<span class="pl-smi">activeElement</span>, {
    <span class="pl-c">// Scroll animation will have the same duration and easing as the keyboard animation.</span>
    <span class="pl-smi">animation</span>,
    paddingBottom: <span class="pl-smi">animation</span>.<span class="pl-smi">endValue</span>,
    <span class="pl-smi">signal</span>,
  });
});
</code></pre><p>Check out <a href="https://github.com/smikhalevski/racehorse/tree/master/web/example/src/App.tsx#L45">the example app<span class="external"></span></a> for real-world keyboard animation handling.</p><br><p align="center"><img src="https://raw.githubusercontent.com/smikhalevski/racehorse/refs/heads/master/images/keyboard-animation.gif" alt="Keyboard animation" width="300"></p><h1 id="network-plugin"><a class="markdown-permalink" href="#network-plugin"><span class="icon icon-link"></span></a>Network plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.NetworkManager.html"><code>NetworkManager</code><span class="external"></span></a> enables network connection monitoring support.</p><p>Initialize the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.NetworkPlugin</span>
...
</code></pre><p>If you are using React, refer to the <a href="https://smikhalevski.github.io/racehorse/functions/_racehorse_react.useNetworkStatus.html"><code>useNetworkStatus</code><span class="external"></span></a> hook, which re-renders a component when the network status changes.</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">useNetworkStatus</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@racehorse/react<span class="pl-pds">'</span></span>;
...
<span class="pl-c">// ⮕ Promise&#x3C;boolean></span>
</code></pre><h1 id="cookbook"><a class="markdown-permalink" href="#cookbook"><span class="icon icon-link"></span></a>Cookbook</h1><h2 id="blur-preview-on-recent-apps-screen"><a class="markdown-permalink" href="#blur-preview-on-recent-apps-screen"><span class="icon icon-link"></span></a>Blur preview on recent apps screen</h2><p>Post a custom <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-notice-event/index.html"><code>NoticeEvent</code><span class="external"></span></a> in <a href="https://developer.android.com/reference/android/app/Activity#onWindowFocusChanged(boolean)"><code>onWindowFocusChanged</code></a>:</p><pre><code class="language-kotlin"><span class="pl-k">package</span> <span class="pl-en">com.myapplication</span>
...
</code></pre><p>In the web app, subscribe to this event and apply a blur filter to the body:</p><pre><code class="language-ts"><span class="pl-smi">eventBridge</span>.<span class="pl-en">subscribe</span>(<span class="pl-s"><span class="pl-pds">'</span>com.myapplication.WindowFocusChangedEvent<span class="pl-pds">'</span></span>, <span class="pl-v">payload</span> <span class="pl-k">=></span> {
...
</code></pre><p>Now your application will appear blurred when it moves to the background and unblurred when it returns to the foreground.</p>`};function l(){return s.createElement(a,{logo:s.createElement(`div`,{style:o(r,n),className:i.Logo,title:`Racehorse`}),readme:c})}export{l as default};