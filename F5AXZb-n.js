...
</code></pre><p>Fields returned by the <a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.FieldAPI.html#at"><code>Field.at</code><span class="external"></span></a> method have a stable identity. This means that you can invoke <code>at(key)</code> with the same key multiple times and the same field instance is returned:</p><pre><code class="language-ts"><span class="pl-smi">universeField</span>.<span class="pl-en">at</span>(<span class="pl-s"><span class="pl-pds">'</span>planets<span class="pl-pds">'</span></span>);
...
</code></pre><p>When a value is set to a child field, a parent field value is also updated. If the parent field doesn't have a value yet, Roqueform would infer its type from a key of the child field.</p><pre><code class="language-ts"><span class="pl-smi">universeField</span>.<span class="pl-c1">value</span>; <span class="pl-c">// ⮕ undefined</span>
...
</code></pre><p>All events conform to the <a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.FieldEvent.html"><code>FieldEvent</code><span class="external"></span></a> interface.</p><p>Without plugins, fields publish only the <a href="https://smikhalevski.github.io/roqueform/types/roqueform.FieldEventType.html"><code>valueChanged</code><span class="external"></span></a> event when the field value is changed via <a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.FieldAPI.html#setvalue"><code>Field.setValue</code><span class="external"></span></a>.</p><p>The root field and its descendants are updated before the <code>valueChanged</code> event is published, so it's safe to read field values in a listener.</p><p>Fields use <a href="https://262.ecma-international.org/7.0/#sec-samevaluezero">SameValueZero<span class="external"></span></a> comparison to detect that the value has changed.</p><pre><code class="language-ts"><span class="pl-smi">planetsField</span>
...
</code></pre><p>To propagate the transient value contained by the child field to its parent, use the <a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.FieldAPI.html#flushtransient"><code>Field.flushTransient</code><span class="external"></span></a> method:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">at</span>(<span class="pl-s"><span class="pl-pds">'</span>hello<span class="pl-pds">'</span></span>).<span class="pl-en">flushTransient</span>();
...
</code></pre><p><a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.FieldAPI.html#settransientvalue"><code>Field.setTransientValue</code><span class="external"></span></a> can be called multiple times, but only the most recent update is propagated to the parent field after the <code>Field.flushTransient</code> call.</p><p>When a child field is in a transient state, its value as seen from the parent may differ from the actual value:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">planetsField</span> <span class="pl-k">=</span> <span class="pl-en">createField</span>([<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span>]);
...
</code></pre><h1 id="accessors"><a class="markdown-permalink" href="#accessors"><span class="icon icon-link"></span></a>Accessors</h1><p><a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.ValueAccessor.html"><code>ValueAccessor</code><span class="external"></span></a> creates, reads, and updates field values.</p><ul><li><p>When the child field is accessed via <a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.FieldAPI.html#at"><code>Field.at</code><span class="external"></span></a> for the first time, its value is read from the value of the parent field using the <a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.ValueAccessor.html#get"><code>ValueAccessor.get</code><span class="external"></span></a> method.</p></li><li><p>When a field value is updated via <a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.FieldAPI.html#setvalue"><code>Field.setValue</code><span class="external"></span></a>, then the parent field value is updated with the value returned from the <a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.ValueAccessor.html#set"><code>ValueAccessor.set</code><span class="external"></span></a> method. If the updated field has child fields, their values are updated with values returned from the <a href="https://smikhalevski.github.io/roqueform/interfaces/roqueform.ValueAccessor.html#get"><code>ValueAccessor.get</code><span class="external"></span></a> method.</p></li></ul><p>By default, Roqueform uses <a href="https://smikhalevski.github.io/roqueform/variables/roqueform.naturalValueAccessor.html"><code>naturalValueAccessor</code><span class="external"></span></a> which supports:</p><ul><li>plain objects,</li><li>class instances,</li><li>arrays,</li><li><code>Map</code>-like instances,</li><li><code>Set</code>-like instances.</li></ul><p>If the field value object has <code>add()</code> and <code>[Symbol.iterator]()</code> methods, it is treated as a <code>Set</code> instance:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">usersField</span> <span class="pl-k">=</span> <span class="pl-en">createField</span>(<span class="pl-k">new</span> <span class="pl-c1">Set</span>([<span class="pl-s"><span class="pl-pds">'</span>Bill<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Rich<span class="pl-pds">'</span></span>]));
...
</code></pre><p>A plugin receives a mutable field instance and should enrich it with additional functionality. To illustrate how plugins work, let's create a simple plugin that enriches a field with a DOM element reference.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">FieldPlugin</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="annotations-plugin"><a class="markdown-permalink" href="#annotations-plugin"><span class="icon icon-link"></span></a>Annotations plugin</h1><p><a href="https://smikhalevski.github.io/roqueform/modules/plugin_annotations.html"><code>annotationsPlugin</code><span class="external"></span></a> associates arbitrary data with fields.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">createField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
...
<span class="pl-smi">field</span>.<span class="pl-en">at</span>(<span class="pl-s"><span class="pl-pds">'</span>hello<span class="pl-pds">'</span></span>).<span class="pl-smi">annotations</span>.<span class="pl-smi">isDisabled</span>; <span class="pl-c">// ⮕ false</span>
</code></pre><p>Annotate a field and all of its children recursively:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">annotate</span>({ isDisabled: <span class="pl-c1">true</span> }, { isRecursive: <span class="pl-c1">true</span> });
...
</code></pre><p>Annotations can be updated using a callback. This is especially useful in conjunction with the recursive flag:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">annotate</span>(
...
</code></pre><h1 id="errors-plugin"><a class="markdown-permalink" href="#errors-plugin"><span class="icon icon-link"></span></a>Errors plugin</h1><p><a href="https://smikhalevski.github.io/roqueform/modules/plugin_errors.html"><code>errorsPlugin</code><span class="external"></span></a> associates errors with fields:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">createField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> <span class="pl-smi">errorsPlugin</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform/plugin/errors<span class="pl-pds">'</span></span>;
...
</code></pre><p>By default, the error type is <code>unknown</code>. To restrict the type of errors that can be added to a field, provide it explicitly:</p><pre><code class="language-ts"><span class="pl-k">interface</span> <span class="pl-en">MyError</span> {
...
</code></pre><p>To add an error to a field, you can publish an <a href="#events-and-subscriptions"><code>errorDetected</code></a> event instead of calling the <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_errors.ErrorsMixin.html#adderror"><code>addError</code><span class="external"></span></a> method:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">publish</span>({
...
</code></pre><p>This is especially useful if you're developing a plugin that adds errors to fields but you don't want to couple it with the errors plugin implementation.</p><p>Subscribe to error changes:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">subscribe</span>(<span class="pl-v">event</span> <span class="pl-k">=></span> {
  <span class="pl-k">if</span> (<span class="pl-c1">event</span>.<span class="pl-c1">type</span> <span class="pl-k">===</span> <span class="pl-s"><span class="pl-pds">'</span>errorAdded<span class="pl-pds">'</span></span>) {
    <span class="pl-c1">event</span>.<span class="pl-c1">target</span>.<span class="pl-smi">errors</span>; <span class="pl-c">// ⮕ MyError[]</span>
  }
});
</code></pre><h1 id="dom-element-reference-plugin"><a class="markdown-permalink" href="#dom-element-reference-plugin"><span class="icon icon-link"></span></a>DOM element reference plugin</h1><p><a href="https://smikhalevski.github.io/roqueform/modules/plugin_ref.html"><code>refPlugin</code><span class="external"></span></a> associates DOM elements with fields.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">createField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="reset-plugin"><a class="markdown-permalink" href="#reset-plugin"><span class="icon icon-link"></span></a>Reset plugin</h1><p><a href="https://smikhalevski.github.io/roqueform/modules/plugin_reset.html"><code>resetPlugin</code><span class="external"></span></a> enhances fields with methods that manage the initial value.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">createField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
...
</code></pre><p>The field is considered dirty when its value differs from the initial value. Values are compared using an equality checker function passed to the <a href="https://smikhalevski.github.io/roqueform/functions/plugin_reset.default.html"><code>resetPlugin</code><span class="external"></span></a>. By default, values are compared using <a href="https://github.com/epoberezkin/fast-deep-equal">fast-deep-equal<span class="external"></span></a>.</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">field</span> <span class="pl-k">=</span> <span class="pl-en">createField</span>({ hello: <span class="pl-s"><span class="pl-pds">'</span>world<span class="pl-pds">'</span></span> }, [<span class="pl-en">resetPlugin</span>()]);
...
</code></pre><h1 id="scroll-to-error-plugin"><a class="markdown-permalink" href="#scroll-to-error-plugin"><span class="icon icon-link"></span></a>Scroll to error plugin</h1><p><a href="https://smikhalevski.github.io/roqueform/modules/plugin_scroll-to-error.html"><code>scrollToErrorPlugin</code><span class="external"></span></a> enhances the field with methods to scroll to the closest invalid field.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">createField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
...
<span class="pl-c">// ⮕ field.at('hello')</span>
</code></pre><p>This plugin works best in conjunction with the <a href="#errors-plugin"><code>errorsPlugin</code></a>. If the invalid field was associated with an element via <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_scroll-to-error.ScrollToErrorMixin.html#ref"><code>ref</code><span class="external"></span></a>, then <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_scroll-to-error.ScrollToErrorMixin.html#scrolltoerror"><code>Field.scrollToError</code><span class="external"></span></a> scrolls the viewport to reveal this element.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">createField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> <span class="pl-smi">errorsPlugin</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform/plugin/errors<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> <span class="pl-smi">scrollToErrorPlugin</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform/plugin/scroll-to-error<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="uncontrolled-plugin"><a class="markdown-permalink" href="#uncontrolled-plugin"><span class="icon icon-link"></span></a>Uncontrolled plugin</h1><p><a href="https://smikhalevski.github.io/roqueform/modules/plugin_uncontrolled.html"><code>uncontrolledPlugin</code><span class="external"></span></a> updates fields by listening to change events of associated DOM elements.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">createField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
...
<span class="pl-smi">field</span>.<span class="pl-en">at</span>(<span class="pl-s"><span class="pl-pds">'</span>hello<span class="pl-pds">'</span></span>).<span class="pl-en">ref</span>(<span class="pl-c1">document</span>.<span class="pl-c1">querySelector</span>(<span class="pl-s"><span class="pl-pds">'</span>input<span class="pl-pds">'</span></span>));
</code></pre><p>The plugin would synchronize the field value with the value of an input element.</p><p>If you have a set of radio buttons or checkboxes that update a single field, call <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_uncontrolled.UncontrolledMixin.html#ref"><code>Field.ref</code><span class="external"></span></a> multiple times providing each element. For example, let's use <code>uncontrolledPlugin</code> to manage an array of animal species:</p><pre><code class="language-html">&#x3C;<span class="pl-ent">input</span> <span class="pl-e">type</span>=<span class="pl-s"><span class="pl-pds">"</span>checkbox<span class="pl-pds">"</span></span> <span class="pl-e">value</span>=<span class="pl-s"><span class="pl-pds">"</span>Elephant<span class="pl-pds">"</span></span> />
...
</code></pre><p>Right after the checkboxes are associated, the input with the value "Zebra" becomes checked. This happens because the <code>uncontrolledPlugin</code> updated the DOM to reflect the current state of the field.</p><p>If the user checks the "Elephant" value, then the field gets updated:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">at</span>(<span class="pl-s"><span class="pl-pds">'</span>animals<span class="pl-pds">'</span></span>).<span class="pl-c1">value</span>; <span class="pl-c">// ⮕ ['Zebra', 'Elephant']</span>
</code></pre><h2 id="value-coercion"><a class="markdown-permalink" href="#value-coercion"><span class="icon icon-link"></span></a>Value coercion</h2><p>By default, <code>uncontrolledPlugin</code> uses an opinionated element value accessor that applies the following coercion rules to values of form elements:</p><table><thead><tr><th>Elements</th><th>Value</th></tr></thead><tbody><tr><td>Single checkbox</td><td><code>boolean</code>, see <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_uncontrolled.ElementsValueAccessorOptions.html#checkboxformat"><code>checkboxFormat</code><span class="external"></span></a>.</td></tr><tr><td>Multiple\xA0checkboxes</td><td>An array of <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/checkbox#value"><code>value</code><span class="external"></span></a> attributes of checked checkboxes, see <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_uncontrolled.ElementsValueAccessorOptions.html#checkboxformat"><code>checkboxFormat</code><span class="external"></span></a>.</td></tr><tr><td>Radio buttons</td><td>The <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/radio#value"><code>value</code><span class="external"></span></a> attribute of a radio button that is checked, or <code>null</code> if no radio buttons are checked.</td></tr><tr><td>Number input</td><td><code>number</code>, or <code>null</code> if empty.</td></tr><tr><td>Range input</td><td><code>number</code></td></tr><tr><td>Date input</td><td>The <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/date#value"><code>value</code><span class="external"></span></a> attribute, or <code>null</code> if empty, see <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_uncontrolled.ElementsValueAccessorOptions.html#dateformat"><code>dateFormat</code><span class="external"></span></a>.</td></tr><tr><td>Time input</td><td>A time string, or <code>null</code> if empty, see <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_uncontrolled.ElementsValueAccessorOptions.html#timeformat"><code>timeFormat</code><span class="external"></span></a>.</td></tr><tr><td>Image input</td><td>A string value of the <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/image#src"><code>value</code><span class="external"></span></a> attribute.</td></tr><tr><td>File input</td><td><a href="https://developer.mozilla.org/en-US/docs/Web/API/File"><code>File</code><span class="external"></span></a> or <code>null</code> if no file is selected; file inputs are read-only.</td></tr><tr><td>Multi-file input</td><td>An array of <a href="https://developer.mozilla.org/en-US/docs/Web/API/File"><code>File</code><span class="external"></span></a>.</td></tr><tr><td>Other</td><td>The <code>value</code> attribute, or <code>null</code> if the element doesn't support it.</td></tr></tbody></table><p><code>null</code>, <code>undefined</code>, <code>NaN</code>, and non-finite numbers are coerced to an empty string and written to the <code>value</code> attribute.</p><p>To change how values are read from and written to the DOM, provide a custom <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_uncontrolled.ElementsValueAccessor.html"><code>ElementsValueAccessor</code><span class="external"></span></a> implementation to a plugin, or use a <a href="https://smikhalevski.github.io/roqueform/functions/plugin_uncontrolled.createElementsValueAccessor.html"><code>createElementsValueAccessor</code><span class="external"></span></a> factory to customise the default behaviour:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">createField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
...
</code></pre><p>Read more about available options in <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_uncontrolled.ElementsValueAccessorOptions.html"><code>ElementsValueAccessorOptions</code><span class="external"></span></a>.</p><h1 id="validation-plugin"><a class="markdown-permalink" href="#validation-plugin"><span class="icon icon-link"></span></a>Validation plugin</h1><p><a href="https://smikhalevski.github.io/roqueform/modules/plugin_validation.html"><code>validationPlugin</code><span class="external"></span></a> enhances fields with validation methods.</p><div class="markdown-alert markdown-alert-tip"><p class="markdown-alert-title"><svg class="octicon octicon-light-bulb mr-2" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="M8 1.5c-2.363 0-4 1.69-4 3.75 0 .984.424 1.625.984 2.304l.214.253c.223.264.47.556.673.848.284.411.537.896.621 1.49a.75.75 0 0 1-1.484.211c-.04-.282-.163-.547-.37-.847a8.456 8.456 0 0 0-.542-.68c-.084-.1-.173-.205-.268-.32C3.201 7.75 2.5 6.766 2.5 5.25 2.5 2.31 4.863 0 8 0s5.5 2.31 5.5 5.25c0 1.516-.701 2.5-1.328 3.259-.095.115-.184.22-.268.319-.207.245-.383.453-.541.681-.208.3-.33.565-.37.847a.751.751 0 0 1-1.485-.212c.084-.593.337-1.078.621-1.489.203-.292.45-.584.673-.848.075-.088.147-.173.213-.253.561-.679.985-1.32.985-2.304 0-2.06-1.637-3.75-4-3.75ZM5.75 12h4.5a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1 0-1.5ZM6 15.25a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 0 1.5h-2.5a.75.75 0 0 1-.75-.75Z"></path></svg>Tip</p><p>This plugin provides low-level functionality. Have a look at <a href="#constraint-validation-api-plugin"><code>constraintValidationPlugin</code></a> or <a href="#schema-plugin"><code>schemaPlugin</code></a> as an alternative.</p></div><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">createField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>roqueform<span class="pl-pds">'</span></span>;
...
</code></pre><p>The <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_validation.Validator.html"><code>Validator</code><span class="external"></span></a> callback receives a <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_validation.Validation.html"><code>Validation</code><span class="external"></span></a> object that references a field where <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_validation.ValidationMixin.html#validate"><code>Field.validate</code><span class="external"></span></a> was called.</p><p>Any result returned from the validator callback is returned from the <code>Field.validate</code> method:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">at</span>(<span class="pl-s"><span class="pl-pds">'</span>hello<span class="pl-pds">'</span></span>).<span class="pl-en">validate</span>();
<span class="pl-c">// ⮕ { ok: boolean }</span>
</code></pre><p>The validator may receive custom options so its behavior can be altered upon each <code>Field.validate</code> call:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">field</span> <span class="pl-k">=</span> <span class="pl-en">createField</span>({ hello: <span class="pl-s"><span class="pl-pds">'</span>world<span class="pl-pds">'</span></span> }, [
...
</code></pre><p>When <code>Field.validate</code> is called, it instantly aborts any pending validation associated with the field. Use <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_validation.Validation.html#abortcontroller"><code>abortController</code><span class="external"></span></a> to detect that a validation was cancelled:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">field</span> <span class="pl-k">=</span> <span class="pl-en">createField</span>({ hello: <span class="pl-s"><span class="pl-pds">'</span>world<span class="pl-pds">'</span></span> }, [
  <span class="pl-en">validationPlugin</span>(<span class="pl-k">async</span> <span class="pl-v">validation</span> <span class="pl-k">=></span> {
...
</code></pre><p><code>Field.validate</code> sets the <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_validation.ValidationMixin.html#validation"><code>validation</code><span class="external"></span></a> property for the field where it was called and for all of its descendants that hold a <a href="#transient-updates">non-transient</a> value:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">validate</span>();
...
<span class="pl-smi">field</span>.<span class="pl-en">at</span>(<span class="pl-s"><span class="pl-pds">'</span>hello<span class="pl-pds">'</span></span>).<span class="pl-smi">isValidating</span>; <span class="pl-c">// ⮕ true</span>
</code></pre><p>Since each field can be validated separately, there can be multiple validations running in parallel. The validator callback can check that a particular field participates in a validation process:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">field</span> <span class="pl-k">=</span> <span class="pl-en">createField</span>({ hello: <span class="pl-s"><span class="pl-pds">'</span>world<span class="pl-pds">'</span></span> }, [
  <span class="pl-en">validationPlugin</span>(<span class="pl-k">async</span> <span class="pl-v">validation</span> <span class="pl-k">=></span> {
...
</code></pre><p>The validation plugin <a href="#events-and-subscriptions">publishes events</a> when the validation state changes:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">subscribe</span>(<span class="pl-v">event</span> <span class="pl-k">=></span> {
...
</code></pre><h1 id="schema-plugin"><a class="markdown-permalink" href="#schema-plugin"><span class="icon icon-link"></span></a>Schema plugin</h1><p><a href="https://smikhalevski.github.io/roqueform/modules/plugin_schema.html"><code>schemaPlugin</code><span class="external"></span></a> enhances fields with validation methods that use a <a href="https://github.com/standard-schema/standard-schema#readme">Standard Schema<span class="external"></span></a> instance to detect validation issues. <code>schemaPlugin</code> uses <a href="#validation-plugin"><code>validationPlugin</code></a> under the hood, so events and validation semantics are exactly the same.</p><p>Any validation library that supports Standard Schema can be used to create a schema object. Let's use <a href="/doubter#readme">Doubter</a> as an example:</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-c1">*</span> <span class="pl-k">as</span> <span class="pl-smi">d</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter<span class="pl-pds">'</span></span>;
...
</code></pre><p>The type of the field value is inferred from the provided schema, so the field value is statically checked.</p><p>When you call the <a href="https://smikhalevski.github.io/roqueform/interfaces/plugin_validation.ValidationMixin.html#validate"><code>Field.validate</code><span class="external"></span></a> method, it triggers validation of the field and all of its child fields:</p><pre><code class="language-ts"><span class="pl-c">// 🟡 Here an invalid value is set to the field</span>
<span class="pl-smi">field</span>.<span class="pl-en">at</span>(<span class="pl-s"><span class="pl-pds">'</span>hello<span class="pl-pds">'</span></span>).<span class="pl-en">setValue</span>(<span class="pl-s"><span class="pl-pds">'</span>universe<span class="pl-pds">'</span></span>);
...
</code></pre><p>Read more about <a href="/doubter#localization">error message localization</a> with Doubter.</p><h1 id="constraint-validation-api-plugin"><a class="markdown-permalink" href="#constraint-validation-api-plugin"><span class="icon icon-link"></span></a>Constraint validation API plugin</h1><p><a href="https://smikhalevski.github.io/roqueform/modules/plugin_constraint-validation.html"><code>constraintValidationPlugin</code><span class="external"></span></a> integrates fields with the <a href="https://developer.mozilla.org/en-US/docs/Web/API/Constraint_validation">Constraint validation API<span class="external"></span></a>.</p><p>For example, let's use the plugin to validate a text input:</p><pre><code class="language-html">&#x3C;<span class="pl-ent">input</span> <span class="pl-e">type</span>=<span class="pl-s"><span class="pl-pds">"</span>text<span class="pl-pds">"</span></span> <span class="pl-e">required</span> />
...
</code></pre><p>Check if the field is invalid:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">at</span>(<span class="pl-s"><span class="pl-pds">'</span>hello<span class="pl-pds">'</span></span>).<span class="pl-smi">isInvalid</span>; <span class="pl-c">// ⮕ true</span>
...
</code></pre><p>Show an error message balloon for the first invalid element and get the field this element is associated with:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-c1">reportValidity</span>();
<span class="pl-c">// ⮕ field.at('hello')</span>
...
<span class="pl-c">// ⮕ [field.at('hello')]</span>
</code></pre><p>Subscribe to field validity changes:</p><pre><code class="language-ts"><span class="pl-smi">field</span>.<span class="pl-en">subscribe</span>(<span class="pl-v">event</span> <span class="pl-k">=></span> {
...
</code></pre><p>The <a href="https://smikhalevski.github.io/roqueform/variables/_roqueform_react.useField.html"><code>useField</code><span class="external"></span></a> hook has the same set of signatures as <a href="https://smikhalevski.github.io/roqueform/functions/roqueform.createField.html"><code>createField</code><span class="external"></span></a>:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">FieldRenderer</span>, <span class="pl-smi">useField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@roqueform/react<span class="pl-pds">'</span></span>;
...
</code></pre><p><code>useField</code> returns a <a href="https://smikhalevski.github.io/roqueform/types/roqueform.Field.html"><code>Field</code><span class="external"></span></a> instance that is preserved between re-renders. The <a href="https://smikhalevski.github.io/roqueform/functions/_roqueform_react.FieldRenderer.html"><code>&#x3C;FieldRenderer></code><span class="external"></span></a> component subscribes to the given field instance and re-renders children when an event is published by the field.</p><p>When a user updates the input value, the <code>rootField.at('hello')</code> value is set and the <code>&#x3C;FieldRenderer></code> component is re-rendered.</p><p>If you pass a callback as an initial value, it will be invoked when the field is initialized.</p><pre><code class="language-ts"><span class="pl-en">useField</span>(() <span class="pl-k">=></span> <span class="pl-en">getInitialValue</span>());
...
</code></pre><h2 id="eager-and-lazy-re-renders"><a class="markdown-permalink" href="#eager-and-lazy-re-renders"><span class="icon icon-link"></span></a>Eager and lazy re-renders</h2><p>Let's consider a form with two <code>&#x3C;FieldRenderer></code> elements. One of them renders the value of the root field and the other one renders an input that updates the child field:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">FieldRenderer</span>, <span class="pl-smi">useField</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@roqueform/react<span class="pl-pds">'</span></span>;
...
</code></pre><p>By default, the <code>&#x3C;FieldRenderer></code> component re-renders only when the provided field was updated directly, meaning updates from ancestors or child fields are ignored. So when the user edits the input value, <code>JSON.stringify</code> won't be re-rendered.</p><p>Add the <a href="https://smikhalevski.github.io/roqueform/interfaces/_roqueform_react.FieldSubscriptionOptions.html#iseagerlyupdated"><code>isEagerlyUpdated</code><span class="external"></span></a> property to force <code>&#x3C;FieldRenderer></code> to re-render whenever its value was affected.</p><pre><code class="language-diff"><span class="pl-md">- &#x3C;FieldRenderer field={rootField}></span>
...
<span class="pl-mi1">+ ></span>
  ...
  &#x3C;/FieldRenderer>
</code></pre><p>Now both fields are re-rendered when the user edits the input text.</p><h2 id="reacting-to-changes"><a class="markdown-permalink" href="#reacting-to-changes"><span class="icon icon-link"></span></a>Reacting to changes</h2><p>Use the <a href="https://smikhalevski.github.io/roqueform/interfaces/_roqueform_react.FieldSubscriptionOptions.html#onchange"><code>onChange</code><span class="external"></span></a> handler, which is triggered only when the field value was updated <a href="#transient-updates">non-transiently</a>.</p><pre><code class="language-tsx">&#x3C;<span class="pl-c1">FieldRenderer</span>
