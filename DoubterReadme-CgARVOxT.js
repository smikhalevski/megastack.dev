...
</code></pre><p>The <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parse"><code>parse</code></a> method takes an input value and returns an output value, or throws a <a href="#validation-errors">validation error</a> if parsing fails:</p><pre><code class="language-ts"><span class="pl-smi">shape</span>.<span class="pl-c1">parse</span>(<span class="pl-c1">42</span>);
...
</code></pre><p>It isn't always convenient to write a try-catch blocks to handle validation errors. Use the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#try"><code>try</code></a> method in such cases:</p><pre><code class="language-ts"><span class="pl-smi">shape</span>.<span class="pl-en">try</span>(<span class="pl-c1">42</span>);
...
</code></pre><p>If you need a fallback value for a nested shape consider using the <a href="#fallback-value"><code>catch</code></a> method.</p><p>For <a href="#async-shapes">asynchronous shapes</a> there's an alternative for each of those methods: <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parseAsync"><code>parseAsync</code></a>, <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#tryAsync"><code>tryAsync</code></a>, and <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parseOrDefaultAsync"><code>parseOrDefaultAsync</code></a>.</p><p>Methods listed in this section can be safely detached from the shape instance:</p><pre><code class="language-ts"><span class="pl-k">const</span> { <span class="pl-c1">parseOrDefault</span> } <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">string</span>();
...
</code></pre><h1 id="validation-errors"><a class="markdown-permalink" href="#validation-errors"><span class="icon icon-link"></span></a>Validation errors</h1><p>Validation errors which are thrown by <a href="#parsing-and-trying">parsing methods</a>, and <a href="https://smikhalevski.github.io/doubter/interfaces/core.Err.html"><code>Err</code></a> objects returned by <code>try</code> and <code>tryAsync</code> methods have the <code>issues</code> property which holds an array of validation issues:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">object</span>({ age: <span class="pl-smi">d</span>.<span class="pl-en">number</span>() });
...
</code></pre><p>The <code>result</code> contains the <a href="https://smikhalevski.github.io/doubter/interfaces/core.Err.html"><code>Err</code></a> object with the array of issues:</p><pre><code class="language-json5">{
...
</code></pre><p>Operations added via <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#addOperation"><code>addOperation</code></a> must return a <a href="https://smikhalevski.github.io/doubter/types/core.Result.html"><code>Result</code></a>:</p><ul><li><code>null</code> if the value is valid and unchanged;</li><li>an <a href="https://smikhalevski.github.io/doubter/interfaces/core.Ok.html"><code>Ok</code></a> object (as in example above) if the value was transformed;</li><li>an array of <a href="https://smikhalevski.github.io/doubter/interfaces/core.Issue.html"><code>Issue</code></a> objects if the operation has failed.</li></ul><p>Multiple operations can be added to shape, and they are executed in the same order they were added. To access all operations that were added use the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#operations"><code>operations</code></a> property.</p><p>In contrast to <a href="#conversions">conversions</a> and <a href="#shape-piping">pipes</a>, operations don't change the base shape. So you can mix them with other operations that belong to the prototype of the base shape:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape2</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>
...
</code></pre><p>The <code>checkUser</code> operation is guaranteed to receive an object, but its properties aren't guaranteed to have correct types.</p><p>Use <a href="https://smikhalevski.github.io/doubter/types/core.OperationOptions.html#tolerance"><code>tolerance</code></a> operation option to change how the operation behaves in case there are issues caused by the shape it is added to:</p><dl><dt><code>"skip"</code></dt><dd><p>If the shape or preceding operations have raised issues, then the operation is skipped but consequent operations are still applied.</p></dd><dt><code>"abort"</code></dt><dd><p>If the shape or preceding operations have raised issues, then the operation is skipped and consequent operations aren't applied. Also, if this operation itself raises issues then consequent operations aren't applied.</p></dd><dt><code>"auto"</code></dt><dd><p>The operation is applied regardless of previously raised issues. This is the default behavior.</p></dd></dl><p>So to make <code>checkUser</code> operation type-safe, we can use "skip" or "abort".</p><pre><code class="language-diff">  const userShape = d
    .object({
      age: d.number(),
...
</code></pre><p>Adding an async operation to the shape, makes shape itself async, so use <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parseAsync"><code>parseAsync</code></a>, <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#tryAsync"><code>tryAsync</code></a>, or <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#parseOrDefaultAsync"><code>parseOrDefaultAsync</code></a>.</p><h2 id="checks"><a class="markdown-permalink" href="#checks"><span class="icon icon-link"></span></a>Checks</h2><p>Checks are the most common <a href="#operations">operations</a> that allow constraining the input value beyond type assertions. For example, if you want to constrain a numeric input to be greater than or equal to 5:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">check</span>(<span class="pl-v">value</span> <span class="pl-k">=></span> {
...
</code></pre><p>A check callback receives the shape output value and must return an issue or an array of issues if the value is invalid. If the value is valid, a check callback must return <code>null</code>, <code>undefined</code>, or an empty array.</p><p>Add asynchronous checks using <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#checkAsync"><code>checkAsync</code></a>. This method has the same semantics as <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#check"><code>check</code></a> but returns a promise and <a href="#async-shapes">makes the shape asynchronous</a>.</p><div class="markdown-alert markdown-alert-note"><p class="markdown-alert-title"><svg class="octicon octicon-info mr-2" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8Zm8-6.5a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13ZM6.5 7.75A.75.75 0 0 1 7.25 7h1a.75.75 0 0 1 .75.75v2.75h.25a.75.75 0 0 1 0 1.5h-2a.75.75 0 0 1 0-1.5h.25v-2h-.25a.75.75 0 0 1-.75-.75ZM8 6a1 1 0 1 1 0-2 1 1 0 0 1 0 2Z"></path></svg>Note</p><p>You can <a href="#operations">parameterize</a> checks and <a href="#tolerance-for-issues">set tolerance for issues</a> the same way as any other operation.</p></div><p>Most shapes have <a href="#built-in-plugins">a set of built-in checks</a>. The check we've just implemented above is called <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#gte"><code>gte</code></a> (greater than equals):</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">gte</span>(<span class="pl-c1">5</span>);
...
</code></pre><p>In the example above, an <a href="https://smikhalevski.github.io/doubter/interfaces/core.Err.html"><code>Err</code></a> object is returned:</p><pre><code class="language-json5">{
  ok<span class="pl-k">:</span> <span class="pl-c1">false</span>,
  issues<span class="pl-k">:</span> [
    {
...
</code></pre><p>Add asynchronous refinements using <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#refineAsync"><code>refineAsync</code></a>. This method has the same semantics as <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#refine"><code>refine</code></a> but returns a promise and <a href="#async-shapes">makes the shape asynchronous</a>.</p><p>Use refinements to <a href="https://www.typescriptlang.org/docs/handbook/2/narrowing.html">narrow</a> the output type of the shape:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">isMarsOrPluto</span>(<span class="pl-v">value</span><span class="pl-k">:</span> <span class="pl-c1">string</span>)<span class="pl-k">:</span> <span class="pl-v">value</span> <span class="pl-k">is</span> <span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span> <span class="pl-k">|</span> <span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span> {
...
<span class="pl-c">// ⮕ StringShape</span>
</code></pre><p>Add asynchronous alterations using <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#alterAsync"><code>alterAsync</code></a>. This method has the same semantics as <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#alter"><code>alter</code></a> but returns a promise and <a href="#async-shapes">makes the shape asynchronous</a>.</p><p>Use any transformation library in conjunction with alternations:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">alter</span>(<span class="pl-c1">Math</span>.<span class="pl-c1">abs</span>).<span class="pl-en">alter</span>(<span class="pl-c1">Math</span>.<span class="pl-c1">pow</span>, { param: <span class="pl-c1">3</span> });
...
</code></pre><p>Throw a <a href="https://smikhalevski.github.io/doubter/classes/core.ValidationError.html"><code>ValidationError</code></a> inside the callback to notify parser that the conversion cannot be successfully completed:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">toNumber</span>(<span class="pl-v">input</span><span class="pl-k">:</span> <span class="pl-c1">string</span>)<span class="pl-k">:</span> <span class="pl-c1">number</span> {
...
</code></pre><p>Refer to <a href="#async-shapes">Async shapes</a> section for more details on when shapes can become asynchronous.</p><h1 id="early-return"><a class="markdown-permalink" href="#early-return"><span class="icon icon-link"></span></a>Early return</h1><p>By default, Doubter collects all issues during parsing. In some cases, you may want to halt parsing and raise a validation error as soon as the first issue was encountered. To do this, pass the <a href="https://smikhalevski.github.io/doubter/interfaces/core.ParseOptions.html#earlyReturn"><code>earlyReturn</code></a> option to the <a href="#parsing-and-trying">parsing methods</a>.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">string</span>()
...
</code></pre><p>This would return the <a href="https://smikhalevski.github.io/doubter/interfaces/core.Err.html"><code>Err</code></a> object with only one issue:</p><pre><code class="language-json5">{
  ok<span class="pl-k">:</span> <span class="pl-c1">false</span>,
  issues<span class="pl-k">:</span> [
    {
      code<span class="pl-k">:</span> <span class="pl-s"><span class="pl-pds">'</span>string.max<span class="pl-pds">'</span></span>,
...
</code></pre><p><a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#annotate"><code>annotate</code></a> returns the clone of the shape with updated annotations. Annotations are merged when you add them:</p><pre><code class="language-ts"><span class="pl-smi">shape</span>.<span class="pl-en">annotate</span>({ foo: <span class="pl-s"><span class="pl-pds">'</span>bar<span class="pl-pds">'</span></span> }).<span class="pl-smi">annotations</span>;
<span class="pl-c">// ⮕ { description: 'Username', foo: 'bar' }</span>
</code></pre><p><a href="#validation-errors">Validation issues</a> have a <a href="https://smikhalevski.github.io/doubter/interfaces/core.Issue.html#meta"><code>meta</code></a> property that you can use to store an arbitrary data.</p><p>You can pass the <a href="https://smikhalevski.github.io/doubter/interfaces/core.IssueOptions.html#meta"><code>meta</code></a> option to any <a href="#checks">built-in check</a> and its value is assigned to the <code>meta</code> property of the raised validation issue.</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">gt</span>(<span class="pl-c1">5</span>, { meta: <span class="pl-s"><span class="pl-pds">'</span>Useful data<span class="pl-pds">'</span></span> });
<span class="pl-c">// ⮕ Shape&#x3C;number></span>
...
</code></pre><p>This comes handy if you want to enhance an issue with an additional data that can be used later during issues processing. For example, during <a href="#localization">localization</a>.</p><h1 id="parsing-context"><a class="markdown-permalink" href="#parsing-context"><span class="icon icon-link"></span></a>Parsing context</h1><p>Inside <a href="#operations">operation</a> callbacks, <a href="#checks">check</a> callbacks, <a href="#refinements">refinement predicates</a>, <a href="#alterations">alteration</a> callbacks, <a href="#conversions">converters</a>, <a href="#fallback-value">fallback</a> functions, and <a href="#localization">message</a> callbacks you can access options passed to the parser. The <a href="https://smikhalevski.github.io/doubter/interfaces/core.ParseOptions.html#context"><code>context</code></a> option may store an arbitrary data, which is <code>undefined</code> by default.</p><p>For example, here's how you can use context to convert numbers to formatted strings:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">convert</span>(
...
</code></pre><h1 id="replace-allow-and-deny-a-value"><a class="markdown-permalink" href="#replace-allow-and-deny-a-value"><span class="icon icon-link"></span></a>Replace, allow, and deny a value</h1><p>All shapes support <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#replace"><code>replace</code></a>, <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#allow"><code>allow</code></a>, and <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#deny"><code>deny</code></a> methods that change how separate literal values are processed.</p><h2 id="replace-a-value"><a class="markdown-permalink" href="#replace-a-value"><span class="icon icon-link"></span></a>Replace a value</h2><p>You can replace an input value with an output value:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape1</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">enum</span>([<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span>]).<span class="pl-c1">replace</span>(<span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Jupiter<span class="pl-pds">'</span></span>);
...
</code></pre><p>To remove a value from this enum you can use the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#deny"><code>deny</code></a> method:</p><pre><code class="language-ts"><span class="pl-smi">shape1</span>.<span class="pl-en">deny</span>(<span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span>);
...
<span class="pl-smi">yesNoShape</span>.<span class="pl-c1">parse</span>(<span class="pl-s"><span class="pl-pds">'</span>true<span class="pl-pds">'</span></span>);
<span class="pl-c">// ❌ ValidationError: type.boolean at /: Must be a boolean</span>
</code></pre><h1 id="introspection"><a class="markdown-permalink" href="#introspection"><span class="icon icon-link"></span></a>Introspection</h1><p>Doubter provides various ways to introspect your shapes at runtime. Let's start by accessing shape input types using the <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#inputs"><code>inputs</code></a> property:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape1</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">or</span>([<span class="pl-smi">d</span>.<span class="pl-en">string</span>(), <span class="pl-smi">d</span>.<span class="pl-en">boolean</span>()]);
...
</code></pre><p>To detect the type of the value use <a href="https://smikhalevski.github.io/doubter/classes/core.Type.html#of"><code>Type.of</code></a>:</p><pre><code class="language-ts"><span class="pl-smi">Type</span>.<span class="pl-en">of</span>(<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>);
...
</code></pre><p>Semantics described above are applied to the <a href="https://smikhalevski.github.io/doubter/interfaces/core.IssueOptions.html#message"><code>message</code></a> option as well:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">string</span>().<span class="pl-en">length</span>(<span class="pl-c1">3</span>, { message: <span class="pl-s"><span class="pl-pds">'</span>Invalid length<span class="pl-pds">'</span></span> })
</code></pre><h2 id="override-default-messages"><a class="markdown-permalink" href="#override-default-messages"><span class="icon icon-link"></span></a>Override default messages</h2><p>Default issue messages can be overridden by <a href="https://smikhalevski.github.io/doubter/interfaces/core.ParseOptions.html#messages"><code>messages</code></a> option:</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-c1">*</span> <span class="pl-k">as</span> <span class="pl-smi">d</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter<span class="pl-pds">'</span></span>;
...
<span class="pl-smi">d</span>.<span class="pl-en">number</span>().<span class="pl-en">gte</span>(<span class="pl-c1">3</span>); <span class="pl-c">// ❌ gte is undefined</span>
</code></pre><h2 id="built-in-plugins"><a class="markdown-permalink" href="#built-in-plugins"><span class="icon icon-link"></span></a>Built-in plugins</h2><ul><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_array_essentials.html"><strong>Array essentials</strong></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#length"><code>length</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#min"><code>min</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#max"><code>max</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#nonEmpty"><code>nonEmpty</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html#includes"><code>includes</code></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_bigint_essentials.html"><strong>Bigint essentials</strong></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#positive"><code>positive</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#negative"><code>negative</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#nonPositive"><code>nonPositive</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#nonNegative"><code>nonNegative</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#min"><code>min</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html#max"><code>max</code></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_date_essentials.html"><strong>Date essentials</strong></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#min"><code>min</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#max"><code>max</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#after"><code>after</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#before"><code>before</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#toISOString"><code>toISOString</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html#toTimestamp"><code>toTimestamp</code></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_number_essentials.html"><strong>Number essentials</strong></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#finite"><code>finite</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#int"><code>int</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#positive"><code>positive</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#negative"><code>negative</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#nonPositive"><code>nonPositive</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#nonNegative"><code>nonNegative</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#between"><code>between</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#gt"><code>gt</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#lt"><code>lt</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#gte"><code>gte</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#lte"><code>lte</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#min"><code>min</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#max"><code>max</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#multipleOf"><code>multipleOf</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html#safe"><code>safe</code></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_object_essentials.html"><strong>Object essentials</strong></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#plain"><code>plain</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#allKeys"><code>allKeys</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#notAllKeys"><code>notAllKeys</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#orKeys"><code>orKeys</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#xorKeys"><code>xorKeys</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#oxorKeys"><code>oxorKeys</code></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_set_essentials.html"><strong>Set essentials</strong></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html#size"><code>size</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html#min"><code>min</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html#max"><code>max</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html#nonEmpty"><code>nonEmpty</code></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_string_essentials.html"><strong>String essentials</strong></a><br><a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#length"><code>length</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#min"><code>min</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#max"><code>max</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#regex"><code>regex</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#includes"><code>includes</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#startsWith"><code>startsWith</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#endsWith"><code>endsWith</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#nonBlank"><code>nonBlank</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#nonEmpty"><code>nonEmpty</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#trim"><code>trim</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#toLowerCase"><code>toLowerCase</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html#toUpperCase"><code>toUpperCase</code></a></p></li><li><p><a href="https://smikhalevski.github.io/doubter/modules/plugin_object_eval.html"><strong>Object eval</strong></a><br>If <code>new Function</code> calls are allowed by the environment, this plugin compiles internal methods of the <code>ObjectShape</code> to boost performance.</p></li></ul><h2 id="recommended-plugins"><a class="markdown-permalink" href="#recommended-plugins"><span class="icon icon-link"></span></a>Recommended plugins</h2><ul><li><a href="https://github.com/smikhalevski/doubter-plugin-string-format#readme">@doubter/plugin-string-format</a><br>Extends <code>StringShape</code> with email, FQDN, MIME, BIC, ISIN, Luhn, and many other format checks.</li></ul><h2 id="integrations"><a class="markdown-permalink" href="#integrations"><span class="icon icon-link"></span></a>Integrations</h2><p>You can combine Doubter with your favourite predicate library using <a href="#refinements">refinements</a>.</p><p>For example, create a shape that validates that input is an email using <a href="https://github.com/validatorjs/validator.js">Validator.js</a>:</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-c1">*</span> <span class="pl-k">as</span> <span class="pl-smi">d</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter<span class="pl-pds">'</span></span>;
...
</code></pre><p>You can use Doubter <a href="#alterations">alterations</a> with various utility libraries, such as <a href="https://lodash.com/">Lodash</a>:</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-c1">*</span> <span class="pl-k">as</span> <span class="pl-smi">d</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter<span class="pl-pds">'</span></span>;
...
</code></pre><h2 id="authoring-a-plugin"><a class="markdown-permalink" href="#authoring-a-plugin"><span class="icon icon-link"></span></a>Authoring a plugin</h2><p>Plugins use <a href="https://www.typescriptlang.org/docs/handbook/declaration-merging.html#module-augmentation">TypeScript's module augmentation</a> to extend functionality of shapes exported from the <a href="https://smikhalevski.github.io/doubter/classes/core.html">doubter/core</a> module.</p><p>Below is an example, how you can implement a naive email check and extend the <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html"><code>StringShape</code></a>.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">StringShape</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>doubter/core<span class="pl-pds">'</span></span>;
...
</code></pre><h2 id="implementing-deep-partial-support"><a class="markdown-permalink" href="#implementing-deep-partial-support"><span class="icon icon-link"></span></a>Implementing deep partial support</h2><p>To enable <code>deepPartial</code> support, your shape must implement <a href="https://smikhalevski.github.io/doubter/interfaces/core.DeepPartialProtocol.html"><code>DeepPartialProtocol</code></a>.</p><pre><code class="language-ts"><span class="pl-k">class</span> <span class="pl-en">MyShape</span>
...
</code></pre><p>This is sufficient to enable type inference and runtime support for <code>deepPartial</code> method.</p><h1 id="performance"><a class="markdown-permalink" href="#performance"><span class="icon icon-link"></span></a>Performance</h1><p>The chart below showcases the performance comparison of Doubter and its peers, in terms of millions of operations per second (greater is better).</p><p align="center"><picture><source media="(prefers-color-scheme: dark)" srcset="./assets/perf-dark.svg"><source media="(prefers-color-scheme: light)" srcset="./assets/perf-light.svg"><img alt="Performance comparison chart" src="./assets/perf-light.svg"></picture></p><p>Tests were conducted using <a href="https://github.com/smikhalevski/toofast#readme">TooFast</a> on Apple M1 with Node.js v20.4.0.</p><p>To reproduce <a href="./src/test/perf/overall.perf.js">the performance test suite</a> results, clone this repo and run:</p><pre><code class="language-shell">npm ci
npm run build
npm run perf -- -t overall
</code></pre><details><summary>Detailed results</summary><pre><code>Success path
...
<span class="pl-c">// ⮕ Shape&#x3C;{ foo: string }></span>
</code></pre><p>Create a shape that is constrained by a <a href="https://www.typescriptlang.org/docs/handbook/2/narrowing.html">narrowing predicate</a>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">any</span>((<span class="pl-v">value</span>)<span class="pl-k">:</span> <span class="pl-v">value</span> <span class="pl-k">is</span> <span class="pl-c1">string</span> <span class="pl-k">=></span> <span class="pl-k">typeof</span> <span class="pl-smi">value</span> <span class="pl-k">===</span> <span class="pl-s"><span class="pl-pds">'</span>string<span class="pl-pds">'</span></span>);
<span class="pl-c">// ⮕ Shape&#x3C;any, string></span>
</code></pre><h1 id="array"><a class="markdown-permalink" href="#array"><span class="icon icon-link"></span></a><code>array</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.array.html"><code>d.array</code></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html"><code>ArrayShape</code></a> instance.</p><p>Constrains a value to be an array:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">array</span>();
<span class="pl-c">// ⮕ Shape&#x3C;any[]></span>
...
</code></pre><h1 id="bigint"><a class="markdown-permalink" href="#bigint"><span class="icon icon-link"></span></a><code>bigint</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.bigint.html"><code>d.bigint</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.BigIntShape.html"><code>BigIntShape</code></a> instance.</p><p>Constrains a value to be a bigint.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">bigint</span>();
...
<span class="pl-c">// ❌ ValidationError: type.bigint at /: Must be a bigint</span>
</code></pre><h1 id="boolean-bool"><a class="markdown-permalink" href="#boolean-bool"><span class="icon icon-link"></span></a><code>boolean</code>, <code>bool</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.boolean.html"><code>d.boolean</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.BooleanShape.html"><code>BooleanShape</code></a> instance.</p><p>Constrains a value to be boolean.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">boolean</span>();
...
<span class="pl-c">// ❌ ValidationError: type.boolean at /: Must be a boolean</span>
</code></pre><h1 id="const"><a class="markdown-permalink" href="#const"><span class="icon icon-link"></span></a><code>const</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.const.html"><code>d.const</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.ConstShape.html"><code>ConstShape</code></a> instance.</p><p>Constrains a value to be an exact value:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">const</span>(<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>);
...
</code></pre><h1 id="convert-convertasync"><a class="markdown-permalink" href="#convert-convertasync"><span class="icon icon-link"></span></a><code>convert</code>, <code>convertAsync</code></h1><p>Both <a href="https://smikhalevski.github.io/doubter/functions/core.convert.html"><code>d.convert</code></a> and <a href="https://smikhalevski.github.io/doubter/functions/core.convertAsync.html"><code>d.convertAsync</code></a> return a <a href="https://smikhalevski.github.io/doubter/classes/core.ConvertShape.html"><code>ConvertShape</code></a> instance.</p><p>Converts the input value:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">convert</span>(<span class="pl-smi">parseFloat</span>);
<span class="pl-c">// ⮕ Shape&#x3C;any, number></span>
...
<span class="pl-c">// ⮕ Shape&#x3C;any, string></span>
</code></pre><p>For more information, see <a href="#conversions">Conversions</a> section.</p><h1 id="date"><a class="markdown-permalink" href="#date"><span class="icon icon-link"></span></a><code>date</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.date.html"><code>d.date</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.DateShape.html"><code>DateShape</code></a> instance.</p><p>Constrains a value to be a valid date.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">date</span>();
...
<span class="pl-c">// ❌ ValidationError: type.date at /: Must be a Date</span>
</code></pre><h1 id="enum"><a class="markdown-permalink" href="#enum"><span class="icon icon-link"></span></a><code>enum</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.enum.html"><code>d.enum</code></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.EnumShape.html"><code>EnumShape</code></a> instance.</p><p>Constrains a value to be equal to one of predefined values:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">enum</span>([<span class="pl-s"><span class="pl-pds">'</span>Mars<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Pluto<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>Jupiter<span class="pl-pds">'</span></span>]);
<span class="pl-c">// ⮕ Shape&#x3C;'Mars', 'Pluto', 'Jupiter'></span>
...
</code></pre><p>Or use <a href="https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-4.html#const-assertions">an object with a <code>const</code> assertion</a>:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">planets</span> <span class="pl-k">=</span> {
...
</code></pre><h1 id="instanceof"><a class="markdown-permalink" href="#instanceof"><span class="icon icon-link"></span></a><code>instanceOf</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.instanceOf.html"><code>d.instanceOf</code></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.InstanceShape.html"><code>InstanceShape</code></a> instance.</p><p>Constrains a value to be an object that is an instance of a class:</p><pre><code class="language-ts"><span class="pl-k">class</span> <span class="pl-en">User</span> {
...
</code></pre><h1 id="lazy"><a class="markdown-permalink" href="#lazy"><span class="icon icon-link"></span></a><code>lazy</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.lazy.html"><code>d.lazy</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.LazyShape.html"><code>LazyShape</code></a> instance.</p><p>With <code>lazy</code> you can declare recursive shapes. To showcase how to use it, let's create a shape that validates JSON data:</p><pre><code class="language-ts"><span class="pl-k">type</span> <span class="pl-en">JSON</span> <span class="pl-k">=</span>
...
</code></pre><p>You can <a href="https://smikhalevski.github.io/doubter/classes/core.LazyShape.html#circular">provide a callback</a> that returns a value that is used as a replacement value for circular references. Or it can throw a <a href="#validation-errors"><code>ValidationError</code></a> from the callback to indicate that circular references aren't allowed:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">userShape3</span><span class="pl-k">:</span> <span class="pl-en">d</span>.<span class="pl-en">Shape</span>&#x3C;<span class="pl-en">User</span>> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">lazy</span>(() <span class="pl-k">=></span>
  <span class="pl-smi">d</span>.<span class="pl-en">object</span>({
...
</code></pre><h1 id="map"><a class="markdown-permalink" href="#map"><span class="icon icon-link"></span></a><code>map</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.map.html"><code>d.map</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.MapShape.html"><code>MapShape</code></a> instance.</p><p>Constrains an input to be a <code>Map</code> instance:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">map</span>(<span class="pl-smi">d</span>.<span class="pl-en">string</span>(), <span class="pl-smi">d</span>.<span class="pl-en">number</span>());
...
</code></pre><h1 id="nan"><a class="markdown-permalink" href="#nan"><span class="icon icon-link"></span></a><code>nan</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.nan.html"><code>d.nan</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.ConstShape.html"><code>ConstShape</code></a> instance.</p><p>The shape that requires an input to be <code>NaN</code>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">nan</span>();
<span class="pl-c">// ⮕ Shape&#x3C;number></span>
...
<span class="pl-c">// ⮕ Shape&#x3C;number></span>
</code></pre><h1 id="never"><a class="markdown-permalink" href="#never"><span class="icon icon-link"></span></a><code>never</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.never.html"><code>d.never</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.NeverShape.html"><code>NeverShape</code></a> instance.</p><p>The shape that always raises a validation issue regardless of an input value:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">never</span>();
<span class="pl-c">// ⮕ Shape&#x3C;never></span>
</code></pre><h1 id="not"><a class="markdown-permalink" href="#not"><span class="icon icon-link"></span></a><code>not</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.not.html"><code>d.not</code></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.ExcludeShape.html"><code>ExcludeShape</code></a> instance.</p><p>The shape that allows any value that doesn't conform the negated shape:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">not</span>(<span class="pl-smi">d</span>.<span class="pl-en">string</span>())
<span class="pl-c">// ⮕ Shape&#x3C;any></span>
...
<span class="pl-c">// ❌ ValidationError: any.exclude at /: Must not conform the excluded shape</span>
</code></pre><p>More about exclusions in the <a href="#exclude-a-shape">Exclude a shape</a> section.</p><h1 id="null"><a class="markdown-permalink" href="#null"><span class="icon icon-link"></span></a><code>null</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.null.html"><code>d.null</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.ConstShape.html"><code>ConstShape</code></a> instance.</p><p>The shape that requires an input to be <code>null</code>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">null</span>();
<span class="pl-c">// ⮕ Shape&#x3C;null></span>
</code></pre><h1 id="number"><a class="markdown-permalink" href="#number"><span class="icon icon-link"></span></a><code>number</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.number.html"><code>d.number</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.NumberShape.html"><code>NumberShape</code></a> instance.</p><p>The shape that requires an input to be a number.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">number</span>();
<span class="pl-c">// ⮕ Shape&#x3C;number></span>
...
<span class="pl-c">// ❌ ValidationError: type.number at /: Must be a number</span>
</code></pre><h1 id="object"><a class="markdown-permalink" href="#object"><span class="icon icon-link"></span></a><code>object</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.object.html"><code>d.object</code></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html"><code>ObjectShape</code></a> instance.</p><p>Constrains a value to be an object with a set of properties:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">object</span>({
  name: <span class="pl-smi">d</span>.<span class="pl-en">string</span>(),
  age: <span class="pl-smi">d</span>.<span class="pl-en">number</span>()
});
<span class="pl-c">// ⮕ Shape&#x3C;{ name: string, age: number }></span>
...
</code></pre><h2 id="key-relationships"><a class="markdown-permalink" href="#key-relationships"><span class="icon icon-link"></span></a>Key relationships</h2><p>Declare relationships between object keys using <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#allKeys"><code>allKeys</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#notAllKeys"><code>notAllKeys</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#orKeys"><code>orKeys</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#xorKeys"><code>xorKeys</code></a> <a href="https://smikhalevski.github.io/doubter/classes/core.ObjectShape.html#oxorKeys"><code>oxorKeys</code></a></p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">shape</span> <span class="pl-k">=</span> <span class="pl-smi">d</span>.<span class="pl-en">object</span>({
  foo: <span class="pl-smi">d</span>.<span class="pl-en">string</span>(),
  bar: <span class="pl-smi">d</span>.<span class="pl-en">number</span>(),
...
</code></pre><h1 id="promise"><a class="markdown-permalink" href="#promise"><span class="icon icon-link"></span></a><code>promise</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.promise.html"><code>d.promise</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.PromiseShape.html"><code>PromiseShape</code></a> instance.</p><p>The shape that checks that an input is an instance of <code>Promise</code>.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">promise</span>();
<span class="pl-c">// ⮕ Shape&#x3C;Promise&#x3C;any>></span>
...
</code></pre><h1 id="record"><a class="markdown-permalink" href="#record"><span class="icon icon-link"></span></a><code>record</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.record.html"><code>d.record</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.RecordShape.html"><code>RecordShape</code></a> instance.</p><p>Constrain keys and values of a dictionary-like object:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">record</span>(<span class="pl-smi">d</span>.<span class="pl-en">number</span>())
...
</code></pre><h1 id="set"><a class="markdown-permalink" href="#set"><span class="icon icon-link"></span></a><code>set</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.set.html"><code>d.set</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.SetShape.html"><code>SetShape</code></a> instance.</p><p>Constrains an input to be a <code>Set</code> instance:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-c1">set</span>(<span class="pl-smi">d</span>.<span class="pl-en">number</span>());
...
</code></pre><h1 id="string"><a class="markdown-permalink" href="#string"><span class="icon icon-link"></span></a><code>string</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.string.html"><code>d.string</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.StringShape.html"><code>StringShape</code></a> instance.</p><p>Constrains a value to be string.</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">string</span>();
<span class="pl-c">// ⮕ Shape&#x3C;string></span>
...
</code></pre><h1 id="tuple"><a class="markdown-permalink" href="#tuple"><span class="icon icon-link"></span></a><code>tuple</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.tuple.html"><code>d.tuple</code></a> returns an <a href="https://smikhalevski.github.io/doubter/classes/core.ArrayShape.html"><code>ArrayShape</code></a> instance.</p><p>Constrains a value to be a tuple where elements at particular positions have concrete types:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">tuple</span>([<span class="pl-smi">d</span>.<span class="pl-en">string</span>(), <span class="pl-smi">d</span>.<span class="pl-en">number</span>()]);
<span class="pl-c">// ⮕ Shape&#x3C;[string, number]></span>
...
<span class="pl-c">// ⮕ Shape&#x3C;[string], readonly [string]></span>
</code></pre><p>Tuples follow <a href="#coerce-to-an-array">array type coercion rules</a>.</p><h1 id="undefined"><a class="markdown-permalink" href="#undefined"><span class="icon icon-link"></span></a><code>undefined</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.undefined.html"><code>d.undefined</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.ConstShape.html"><code>ConstShape</code></a> instance.</p><p>The shape that requires an input to be <code>undefined</code>:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">undefined</span>();
<span class="pl-c">// ⮕ Shape&#x3C;undefined></span>
</code></pre><h1 id="union-or"><a class="markdown-permalink" href="#union-or"><span class="icon icon-link"></span></a><code>union</code>, <code>or</code></h1><p><a href="https://smikhalevski.github.io/doubter/functions/core.union.html"><code>d.union</code></a> returns a <a href="https://smikhalevski.github.io/doubter/classes/core.UnionShape.html"><code>UnionShape</code></a> instance.</p><p>A constraint that allows a value to be one of the given types:</p><pre><code class="language-ts"><span class="pl-smi">d</span>.<span class="pl-en">union</span>([<span class="pl-smi">d</span>.<span class="pl-en">string</span>(), <span class="pl-smi">d</span>.<span class="pl-en">number</span>()]);
<span class="pl-c">// ⮕ Shape&#x3C;string | number></span>
...
</code></pre><p>Read more about <a href="https://smikhalevski.github.io/doubter/classes/core.Shape.html#at"><code>Shape.at</code></a> method in the <a href="#nested-shapes">Nested shapes</a> section. The same approach can be taken to implement writes:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">setItem</span>&#x3C;<span class="pl-en">K</span> <span class="pl-k">extends</span> <span class="pl-k">keyof</span> <span class="pl-en">LocalStorageItems</span>>(<span class="pl-v">key</span><span class="pl-k">:</span> <span class="pl-en">K</span>, <span class="pl-v">value</span><span class="pl-k">:</span> <span class="pl-en">LocalStorageItems</span>[<span class="pl-en">K</span>])<span class="pl-k">:</span> <span class="pl-c1">void</span> {
  <span class="pl-k">const</span> <span class="pl-c1">valueShape</span> <span class="pl-k">=</span> <span class="pl-smi">localStorageItemsShape</span>.<span class="pl-en">at</span>(<span class="pl-smi">key</span>);
...
<span class="pl-c">// ❌ ValidationError: number.gte at /: Must be greater than 0</span>
</code></pre>`};function d(){return s.createElement(n,{logo:s.createElement("div",{style:{...e(p,a),aspectRatio:970/320,backgroundRepeat:"no-repeat",backgroundSize:"contain",maxWidth:"100%",width:"20rem"},title:"Doubter"}),readme:l})}export{d as default};
