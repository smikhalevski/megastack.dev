...
</code></pre><p>Racehorse uses a <a href="https://greenrobot.org/eventbus">Greenrobot EventBus<span class="external"></span></a> to deliver events to subscribers, so bridge must be registered in the event bus:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.greenrobot.eventbus.EventBus</span>
...
</code></pre><p>Note that <code>ShowToastEvent</code> implements <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-web-event/index.html"><code>WebEvent</code><span class="external"></span></a> marker interface. This is the baseline requirement to which events must conform to support marshalling from the web app to Android.</p><p>Now let's add an event subscriber that would receive incoming <code>ShowToastEvent</code> and display a toast:</p><pre><code class="language-kotlin"><span class="pl-k">package</span> <span class="pl-en">com.example</span>
...
npm start
</code></pre><p>Open <code>&#x3C;racehorse>/android</code> in Android Studio and run <code>example</code> app.</p><h1 id="request-response-event-chains"><a class="markdown-permalink" href="#request-response-event-chains"><span class="icon icon-link"></span></a>Request-response event chains</h1><p>In the <a href="#overview">Overview</a> section we used an event that extends a <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-web-event/index.html"><code>WebEvent</code><span class="external"></span></a> interface. Such events don't imply the response. To create a request-response chain at least two events are required:</p><pre><code class="language-kotlin"><span class="pl-k">package</span> <span class="pl-en">com.example</span>
...
</code></pre><p>If an exception is thrown in <code>DeviceModelPlugin.onGetDeviceModel</code>, then promise is <em>rejected</em> with an <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error"><code>Error</code><span class="external"></span></a> instance.</p><h2 id="synchronous-requests"><a class="markdown-permalink" href="#synchronous-requests"><span class="icon icon-link"></span></a>Synchronous requests</h2><p>If all events in the event chain are handled on <a href="https://greenrobot.org/eventbus/documentation/delivery-threads-threadmode/">the posting thread<span class="external"></span></a> on the Android side, then a request can be handled synchronously on the web side. In the <code>DeviceModelPlugin</code> example <code>onGetDeviceModel</code> is called on the posting thread, since we didn't specify a thread mode for <a href="https://github.com/greenrobot/EventBus#eventbus-in-3-steps"><code>@Subscribe</code><span class="external"></span></a> annotation. So this allows web to perform a synchronous request:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">eventBridge</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>racehorse<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="activity-plugin"><a class="markdown-permalink" href="#activity-plugin"><span class="icon icon-link"></span></a>Activity plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.ActivityManager.html"><code>ActivityManager</code><span class="external"></span></a> starts activities and provides info about the activity that renders the WebView.</p><p>Add Lifecycle dependency to your Android app:</p><pre><code class="language-kotlin">dependencies {
...
</code></pre><p>If you are using React, then refer to <a href="https://smikhalevski.github.io/racehorse/functions/_racehorse_react.useActivityState.html"><code>useActivityState</code><span class="external"></span></a> hook that re-renders a component when activity state changes.</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">useActivityState</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@racehorse/react<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="biometric-encrypted-storage-plugin"><a class="markdown-permalink" href="#biometric-encrypted-storage-plugin"><span class="icon icon-link"></span></a>Biometric encrypted storage plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.BiometricEncryptedStorageManager.html"><code>BiometricEncryptedStorageManager</code><span class="external"></span></a> enables a file-based persistence of a biometric-protected data.</p><p>Add <a href="https://developer.android.com/jetpack/androidx/releases/biometric#declaring_dependencies">Biometric<span class="external"></span></a> dependency to your Android app:</p><pre><code class="language-kotlin">dependencies {
    implementation(<span class="pl-s"><span class="pl-pds">"</span>androidx.biometric:biometric:1.2.0-alpha05<span class="pl-pds">"</span></span>)
}
...
</code></pre><p>To allow device credential authentication, provide <a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.BiometricConfig.html#authenticationvalidityduration"><code>authenticationValidityDuration</code><span class="external"></span></a> that is greater or equal to 0:</p><pre><code class="language-ts"><span class="pl-k">await</span> <span class="pl-smi">biometricEncryptedStorageManager</span>.<span class="pl-c1">set</span>(<span class="pl-s"><span class="pl-pds">'</span>foo<span class="pl-pds">'</span></span>, <span class="pl-s"><span class="pl-pds">'</span>bar<span class="pl-pds">'</span></span>, {
...
</code></pre><h1 id="contacts-plugin"><a class="markdown-permalink" href="#contacts-plugin"><span class="icon icon-link"></span></a>Contacts plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.ContactsManager.html"><code>ContactsManager</code><span class="external"></span></a> provides access to contacts stored on the device.</p><p>Add contacts permission to the app manifest:</p><pre><code class="language-xml">
...
</code></pre><h1 id="deep-link-plugin"><a class="markdown-permalink" href="#deep-link-plugin"><span class="icon icon-link"></span></a>Deep link plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.DeepLinkManager.html"><code>DeepLinkManager</code><span class="external"></span></a> provides access to deep links inside yor web app.</p><p>Initialize the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.DeepLinkPlugin</span>

<span class="pl-en">EventBus</span>.getDefault().register(<span class="pl-en">DeepLinkPlugin</span>())
</code></pre><p>Override <a href="https://developer.android.com/reference/android/app/Activity#onNewIntent(android.content.Intent)"><code>onNewIntent</code></a> in the main activity of yor app and post the deep link event:</p><pre><code class="language-kotlin"><span class="pl-k">override</span> <span class="pl-k">fun</span> <span class="pl-en">onNewIntent</span>(<span class="pl-smi">intent</span><span class="pl-k">:</span> <span class="pl-en">Intent</span>) {
...
</code></pre><h1 id="device-plugin"><a class="markdown-permalink" href="#device-plugin"><span class="icon icon-link"></span></a>Device plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.DeviceManager.html"><code>DeviceManager</code><span class="external"></span></a> provides access to various device settings.</p><p>Add compat library dependency, it is used for window insets acquisition:</p><pre><code class="language-kotlin">dependencies {
...
</code></pre><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.Download.html"><code>Download</code><span class="external"></span></a> instance carries the download status, progress, and file details.</p><p>A storage permission must be added to support Android devices with API level &#x3C;= 29:</p><pre><code class="language-xml">
...
<span class="pl-c">// ⮕ 'bar'</span>
</code></pre><h1 id="evergreen-plugin"><a class="markdown-permalink" href="#evergreen-plugin"><span class="icon icon-link"></span></a>Evergreen plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.EvergreenManager.html"><code>EvergreenManager</code><span class="external"></span></a> provides a way to update your app using an archive that is downloadable from your server.</p><p>You can find an extensive demo of evergreen plugin usage <a href="#example-app">in the example app.</a></p><p>Init the plugin and start the update download process:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">android.os.Bundle</span>
...
</code></pre><h1 id="google-sign-in-plugin"><a class="markdown-permalink" href="#google-sign-in-plugin"><span class="icon icon-link"></span></a>Google Sign-In plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.GoogleSignInManager.html"><code>GoogleSignInManager</code><span class="external"></span></a> enables Google Sign-In support.</p><p>Go to <a href="https://console.firebase.google.com">console.firebase.google.com<span class="external"></span></a>, set up a new project, and configure an Android app following all instructions. Use the <code>applicationId</code> of your app and SHA-1 that is used for app signing. You can use gradle to retrieve SHA-1:</p><pre><code class="language-shell">./gradlew signingReport
</code></pre><p>Go to <a href="https://console.cloud.google.com/apis/credentials">Google Cloud Console<span class="external"></span></a> for your project and add an OAuth client ID for Android.</p><p>Add Google Sign-In SDK dependencies to your Android app:</p><pre><code class="language-kotlin">dependencies {
...
</code></pre><h1 id="keyboard-plugin"><a class="markdown-permalink" href="#keyboard-plugin"><span class="icon icon-link"></span></a>Keyboard plugin</h1><p><a href="https://smikhalevski.github.io/racehorse/interfaces/racehorse.KeyboardManager.html"><code>KeyboardManager</code><span class="external"></span></a> toggles the software keyboard and notifies about keyboard animation.</p><p>Initialize the plugin in your Android app:</p><pre><code class="language-kotlin"><span class="pl-k">import</span> <span class="pl-smi">org.racehorse.KeyboardPlugin</span>
...
</code></pre><p>Use <a href="https://smikhalevski.github.io/racehorse/functions/racehorse.runAnimation.html"><code>runAnimation</code><span class="external"></span></a> to run the animation. For example, if your <a href="https://developer.android.com/develop/ui/views/layout/edge-to-edge">app is rendered edge-to-edge<span class="external"></span></a>, you can animate the bottom padding to compensate the height of the keyboard.</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">useKeyboardAnimation</span>, <span class="pl-smi">runAnimation</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@racehorse/react<span class="pl-pds">'</span></span>;
...
</code></pre><p>If you are using React, then refer to <a href="https://smikhalevski.github.io/racehorse/functions/_racehorse_react.useNetworkStatus.html"><code>useNetworkStatus</code><span class="external"></span></a> hook that re-renders a component when network status changes.</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">useNetworkStatus</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>@racehorse/react<span class="pl-pds">'</span></span>;
...
<span class="pl-c">// ⮕ Promise&#x3C;boolean></span>
</code></pre><h1 id="cookbook"><a class="markdown-permalink" href="#cookbook"><span class="icon icon-link"></span></a>Cookbook</h1><h2 id="blur-preview-on-recent-apps-screen"><a class="markdown-permalink" href="#blur-preview-on-recent-apps-screen"><span class="icon icon-link"></span></a>Blur preview on recent apps screen</h2><p>Post a custom <a href="https://smikhalevski.github.io/racehorse/android/racehorse/org.racehorse/-notice-event/index.html"><code>NoticeEvent</code><span class="external"></span></a> event in <a href="https://developer.android.com/reference/android/app/Activity#onWindowFocusChanged(boolean)"><code>onWindowFocusChanged</code></a>:</p><pre><code class="language-kotlin"><span class="pl-k">package</span> <span class="pl-en">com.myapplication</span>
